...
type
  TForm_OutboundOrder = class(TForm)
    PMain: TPanel;
    FQ0: TFDQuery;
    PT0: TPanel;
    Panel3: TPanel;
...
procedure TForm_OutboundOrder.BtSubmitClick(Sender: TObject);
var
    oFQMain     : TFDQuery;
    iOrderId    : Integer;
begin
    ...
    if oFQMain.IsEmpty then begin
        dwMessage('出库商品不能为空!','error',self);
        Exit;
    end;

    //将当前出库单数据(单条记录)写入 出库单数据表
    FQ0.Close;
    FQ0.SQL.Text := 'INSERT INTO eOutBoundOrder(oUserId,oNo,oDate,oReceiveUnitID,oHandlerId,oVerifierId,oCanvasId)'
        +' OUTPUT inserted.oId'
    ...
    //将出库单的商品详情写入 出库单 详情表
    FQ0.Close;
    FQ0.SQL.Text := 'INSERT INTO eOutBoundOrderdetail (dOrderID,dGoodsId,dQuantity,dPrice,dAmount)'
            +' SELECT :OrderId,tgid,tQuantity,tgInPrice,tAmount FROM eOutBoundOrdertemp WHERE tCanvasID='''+gsCanvasID+'''';
    FQ0.Params.ParamByName('OrderId').Value := iOrderId;
    FQ0.ExecSQL;

    //<将出库单的商品详情更新 库存表eGoods (如果仓库一致,则增加数量;如果不一致,则增加记录)
    FQTemp.Disconnect();
    FQTemp.close;
    FQTemp.Open('SELECT * FROM eOutBoundOrderTemp WHERE tCanvasID='''+gsCanvasID+'''');
    while not FQTemp.Eof do begin
        //
        FQGoods.Disconnect();
        FQGoods.close;
        FQGoods.SQL.Text    := 'UPDATE eGoods Set gQuantity = gQuantity - '+FQTemp.FieldByName('tQuantity').AsString
            +' WHERE gId='+FQTemp.FieldByName('tgId').AsString;
        FQGoods.ExecSQL;
    ...
    //删除当前出库单内的数据记录
    FQ0.Close;
    FQ0.SQL.Text := 'DELETE FROM eOutBoundOrderTemp WHERE tCanvasID='''+gsCanvasID+'''';
    FQ0.ExecSQL;
    ...
    //
    dwMessage('*****     出库成功!     *****','success',self);

end;

procedure TForm_OutboundOrder.BtACancelClick(Sender: TObject);
begin
    PaAdd.Visible   := False;
end;

procedure TForm_OutboundOrder.BtAddClick(Sender: TObject);
begin
    PaAdd.Visible   := True;

end;

procedure TForm_OutboundOrder.BuAOKClick(Sender: TObject);
var
    oFQAdd      : TFDQuery;
...
begin
    ...
    FQ0.Disconnect();
    FQ0.Close;
    FQ0.SQL.Text := 'INSERT INTO eOutBoundOrderTemp (tgId,tgName,tgCode,tgCategoryId,tgGrade,tgBrand,tgSpecification,'
...
procedure TForm_OutboundOrder.CBCChange(Sender: TObject);
begin
...
procedure TForm_OutboundOrder.CBPChange(Sender: TObject);
begin
    //说明:当选择时,自动设置其tag为对应的id
...
procedure TForm_OutboundOrder.CbRChange(Sender: TObject);
begin
    //说明:当选择时,自动设置其tag为对应的id
    ...
    if Trim(CBR.Text) = '' then begin
        ...
        FQ0.Disconnect();
        FQ0.Close;
        FQ0.Open('SELECT rId FROM eReceiveUnit WHERE rName='''+CBR.Text+'''');
        CBR.Tag := FQ0.Fields[0].AsInteger;
    end;
end;

procedure TForm_OutboundOrder.FormShow(Sender: TObject);
begin
    //设置查询组件连接
    ...
    //<先删除所有eOutBoundOrderTemp表中所有同canvasid的记录
    //取得canvasid（浏览器唯一值）
    ...
    //添加选项
    dwGetComboBoxItems(FQ0,'eReceiveUnit','rName',true,CBR);
    dwGetComboBoxItems(FQ0,'eUser','uName',true,CBP);
    dwGetComboBoxItems(FQ0,'eUser','uName',true,CBC);

    //生成默认编号
    EtNo.Text   := 'CK'+FormatDateTime('YYYYMMDDhhmmsszzz',Now);

end;

procedure TForm_OutboundOrder.PMainDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState;
  var Accept: Boolean);
begin
