...
procedure TForm_xxx.B_NextClick(Sender: TObject);
begin
    FDQuery1.Next;
    Edit1.Text  := FDQuery1.FieldByName('uName').AsString;
end;

procedure TForm_xxx.B_PrevClick(Sender: TObject);
begin
    FDQuery1.Prior;
    Edit1.Text  := FDQuery1.FieldByName('uName').AsString;
end;

procedure TForm_xxx.FormShow(Sender: TObject);
begin
    //
    FDQuery1.Connection := TForm1(self.Owner).FDConnection1;
    //
    FDQuery1.Close;
    FDQuery1.SQL.Text   := 'SELECT * FROM dwUser';
    FDQuery1.Open;
    Edit1.Text  := FDQuery1.FieldByName('uName').AsString;
end;
