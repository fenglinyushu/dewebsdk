...
uses
    ...
    //第三方，用于GetMethod
    IdCustomHTTPServer, IdHashSHA, IdGlobal, IdHTTP, IdSSLOpenSSL, IdMultipartFormData,IdURI,
...
type
  TForm_bmap = class(TForm)
    ...
    procedure FormMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X,
      Y: Integer);
    procedure B_GetNameClick(Sender: TObject);
...
procedure TForm_bmap.BtStyleClick(Sender: TObject);
var
    sJS     : string;
begin

    sJS :=
        dwFullName(Shape1)+'__map.setMapStyle({'
            +'styleJson: ['
                +'{'
                    +'"featureType": "all",'
                    +'"elementType": "all",'
...
procedure TForm_bmap.Button10Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
...
procedure TForm_bmap.Button11Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
...
procedure TForm_bmap.Button12Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
		...
		+'          window.lushu = new BMapLib.LuShu('+dwFullName(Shape1)+'__map,arrPois,{'
...
procedure TForm_bmap.Button13Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     := 'window.lushu.start();';

    //
    dwRunJS(sJS,Self);
end;

procedure TForm_bmap.Button14Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     := 'window.lushu.pause();';

    //
    dwRunJS(sJS,Self);
end;

procedure TForm_bmap.Button15Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     := 'window.lushu.stop();';

    //
    dwRunJS(sJS,Self);
end;

procedure TForm_bmap.Button16Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
		'var point = new BMap.Point(108.954, 34.266);'
...
procedure TForm_bmap.Button17Click(Sender: TObject);
var
    sJS     : string;
begin
    Dec(giZoom);
...
procedure TForm_bmap.Button18Click(Sender: TObject);
var
    sJS     : string;
begin
    Inc(giZoom);
...
procedure TForm_bmap.Button1Click(Sender: TObject);
begin
    dwRunJS(dwFullName(Shape1)+'__map.addControl(new BMap.NavigationControl());',self);
end;

procedure TForm_bmap.Button2Click(Sender: TObject);
var
    sJS     : string;
begin
...
procedure TForm_bmap.Button3Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
...
procedure TForm_bmap.Button4Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
...
procedure TForm_bmap.Button5Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
...
procedure TForm_bmap.Button6Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
...
procedure TForm_bmap.Button7Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
...
procedure TForm_bmap.Button8Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
...
procedure TForm_bmap.Button9Click(Sender: TObject);
var
    sJS     : string;
begin
    sJS     :=
...
procedure TForm_bmap.B_GetNameClick(Sender: TObject);
var
    sJS     : string;
begin

    sJS :=
            //'var map = new BMap.Map("allmap");'#13
            'var point = new BMap.Point('+Label1.Caption+');'#13

            // 创建地址解析器实例
            +'var geocoder = new BMap.Geocoder();'#13

            // 将坐标转换成地址
            +'geocoder.getLocation(point, function (rs){'#13
                +'var address = rs.addressComponents;'#13

                // 更新显示结果
                +'alert("当前位置：" + address.province + ", " + address.city + ", " + address.district + ", " + address.street + ", " + address.streetNumber);'#13
            +'});';

    dwRunJS(sJS,self);
end;

procedure TForm_bmap.FormCreate(Sender: TObject);
begin
    giZoom  := 12;
end;

procedure TForm_bmap.FormMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
var
    sJS     : string;
    sHd     : string;
begin
(*
    //原来的启动时指定一个中心位置及缩放， 2024-01-17改为读取当前位置
    sJS     :=
		'var point = new BMap.Point(106.954, 34.266);'
...
*)
    ...
    sJS     :=
            ...
            //增加图层类型选择
            +dwFullName(Shape1)+'__map.addControl(new BMap.MapTypeControl());        '#13#10
            ...
            +'function showmap(position) {                                       '#13#10
                ...
                //更新地图位置
                +'that.'+dwFullName(Label1)+'__cap = cords.longitude + '' , '' + cords.latitude; '#13#10    //将位置信息赋给label1（前端）
                +'that.dwevent("","'+dwFullName(Label1)+'","this.'+dwFullName(Label1)+'__cap","_update",'+sHd+');'#13#10    //更新Label1服务端的值，并激活Onclick事件

                //移动地图的当前定位位置
		        +'var point = new BMap.Point(cords.longitude, cords.latitude);   '#13#10
                +dwFullName(Shape1)+'__map.centerAndZoom(point, '+IntToStr(giZoom)+');'
...
procedure TForm_bmap.FormMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
    dwSetPCMode(Self);
end;
