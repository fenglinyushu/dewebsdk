...
type
    ...
    procedure EKeywordKeyPress(Sender: TObject; var Key: Char);
  private
  public
        //变量---------------
        gsUserInfo      : string;
        ...
        gsPlatform      : String;           //平台，默认为空，即未知，可选项为 pc/mobile

        //子窗体-------------
        Form_BMap       : TForm_Bmap;       //百度地图
        Form_Book       : TForm_Book;       //会议室预定
        Form_CarNo      : TForm_CarNo;      //车牌输入
        Form_Coupon     : TForm_Coupon;     //优惠券
        Form_dbHello    : TForm_dbHello;    //数据库操作入门
        Form_driver     : TForm_driver;     //驾校科目一
...
begin
    try
        ...
        CP0.ActiveCard      := oCard;
        oCard.CardVisible   := True;
        oCard.Name          := 'CD_'+AClass.ClassName;
        ...
        dwMessage('error when dmShowForm!','error',self);
    end;
end;
...
procedure TForm1.EKeywordKeyPress(Sender: TObject; var Key: Char);
begin
...
begin
    ...
    for iItem := 0 to FP0.ControlCount - 1 do begin
        ...
        opanel.Hint := '{"src":"media/images/48/'+IntToStr(oPanel.Tag)+'.png"}';
    end;
...
begin
    ...
    //关闭打开的Form
    if gjoHistory._(gjoHistory._Count-1) = 'showform' then begin
        //删除最后一条历史记录
        gjoHistory.delete(gjoHistory._Count-1);
...
begin
    //在当前页面打开通用登录程序gLogin
    dwOpenUrl(Self,'gLogin?dwMobilePro','_self');
end;
...
begin
    //为浏览器记录变量添加一条记录
    gjoHistory.Add('showform');

    //打开
    dmShowForm(TForm_CheckIn,TForm(Form_CheckIn));
...
begin
    ...
    //打开
    dmShowForm(TForm_qr,TForm(Form_qr));
...
begin
    ...
    //打开
    dmShowForm(TForm_Book,TForm(Form_Book));
...
begin
    ...
    //打开
    dmShowForm(TForm_Coupon,TForm(Form_Coupon));
end;
...
begin
    ...
    //打开
    dmShowForm(TForm_dbHello,TForm(Form_dbHello));
...
begin
    ...
    //打开
    dmShowForm(TForm_driver,TForm(Form_driver));
end;
...
procedure TForm1.PBMapClick(Sender: TObject);
begin
    ...
    //打开扫一扫Form
    dmShowForm(TForm_bmap,TForm(Form_bmap));
end;
