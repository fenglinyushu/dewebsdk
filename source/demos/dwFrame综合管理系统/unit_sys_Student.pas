...
begin
    ...
    FDQuery1.Open(
        ...
        SELECT DISTINCT
            ...
            sGradeNum,
            sClassNum,
            SUBSTRING( sClassName, 1, CHARINDEX ( '级', sClassName ) ) AS GradePart,
            SUBSTRING( sClassName, CHARINDEX ( '级', sClassName ) + 1, LEN ( sClassName ) ) AS ClassPart,
            CAST( REPLACE ( SUBSTRING( sClassName, 3, CHARINDEX ( '级', sClassName ) - 3 ), '初中', '' ) AS INT ) AS YearNumber,
            CAST(
            REPLACE ( SUBSTRING( sClassName, CHARINDEX ( '级', sClassName ) + 1, LEN ( sClassName ) ), '班', '' ) AS INT
            ) AS ClassNumber
        FROM
            sys_Student
            ) AS T
    ...
    while not FDQuery1.Eof do begin
        ...
        if sOldGrade <> FDQuery1.FieldByName('GradePart').AsString then begin
            ...
            //更新SQL
            sSQL    := sSQL + 'INSERT INTO sys_Department(dNo,dGradeNum,dNumber,dName)'
                    +' VALUES('
                        +''''+Format('01%.2d',[iGradeId])+''''
                        +','''+FDQuery1.FieldByName('sGradeNum').AsString+''''
                        +','''+FDQuery1.FieldByName('sGradeNum').AsString+''''
                        +','''+sOldGrade+''''
                    +');';
        ...
        sSQL    := sSQL + 'INSERT INTO sys_Department(dNo,dGradeNum,dNumber,dName)'
                +' VALUES('
                    +''''+Format('01%.2d%.2d',[iGradeId-1,iClassId])+''''
                    +','''+FDQuery1.FieldByName('sGradeNum').AsString+''''
                    +','''+FDQuery1.FieldByName('sClassNum').AsString+''''
                    +','''+FDQuery1.FieldByName('ClassPart').AsString+''''
                +');';
