    ...
    begin
        ...
        try
            ...
            for iiGroup := 0 to Ceil(FDQuery1.RecordCount / SECount.Value)-1 do begin   //iGroup为分组的循环变量,0开始
                ...
                //创建一个组, 返回组id
                FDQuery2.Close;
                FDQuery2.SQL.Text := 'INSERT INTO bop_Test(tName,tDate, tGradeNum, tGender,tItemName,tItemUnit,tLocation,tStatus)'
                    +' OUTPUT inserted.tId'
                    +' VALUES(''temp'',:Date,:GradeNum, :Gender,:ItemName,:ItemUnit,:Location,0)';
                FDQuery2.ParamByName('Date').AsDateTime     := Now;
                FDQuery2.ParamByName('GradeNum').AsInteger  := AGender;//=1,'男','女');
