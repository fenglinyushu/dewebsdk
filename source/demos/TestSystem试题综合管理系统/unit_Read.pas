...
begin
    ...
    FDQUery_Tmp.FieldByName('pname').AsString       := gjoData.aname;
...
begin
    ...
    //创建考试信息
    gjoData         := _json('{}');
    gjoData.aname   := TButton(Sender).Caption;         //试卷名称
    gjoData.id      := TButton(Sender).HotImageIndex;   //id
    gjoData.aorder  := TButton(Sender).tag;             //AOrder
    gjoData.data    := _json('[]');
    while not FDQuery1.Eof do begin
        joData          := _json('{}');
        joData.paperid  := FDQuery1.FieldByName('paperid').AsInteger;   //试卷aorder
        joData.topicid  := FDQuery1.FieldByName('topicid').AsInteger;   //大题aorder
        joData.aorder   := FDQuery1.FieldByName('aorder').AsInteger;    //小题aorder
        joData.aname    := FDQuery1.FieldByName('aname').AsString;      //题干
        joData.atype    := FDQuery1.FieldByName('atype').AsString;      //题型
        joData.ascore   := FDQuery1.FieldByName('ascore').AsInteger;    //小题分数
...
begin
    ...
    while not FDQuery1.Eof do begin
        ...
        oButton.HotImageIndex   := FDQuery1.FieldByName('id').AsInteger;
...
begin
    ...
    if sType = '单选' then begin
        ...
        //
        RB_D.Visible    := FDQuery1.FieldByName('AnswerE').AsString <> '';
        ...
        //
        CK_D.Visible    := FDQuery1.FieldByName('AnswerE').AsString <> '';
