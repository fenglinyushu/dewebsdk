...
//一键生成CRUD模块
function  cpInit(APanel:TPanel;AConnection:TFDConnection;AMobile:Boolean;AReserved:String):Integer;
...
begin
    ...
    sPrimary    := AConfig.primarykey;  //主键字段名称, 应该为自增型int字段
    ...
    //==================================================================================================================
    //先从APanel中取得上次查询的相关信息
    joStyleName := _json(ASG.StyleName);
    if joStyleName = unassigned then begin
        ...
        iOldPageNo  := dwGetInt(joStyleName,'oldpageno');
        iFirstPri   := dwGetInt(joStyleName,'firstprimary');
        iLastPri    := dwGetInt(joStyleName,'lastprimary');
        sFirstSort  := dwGetStr(joStyleName,'firstsort');
        sLastSort   := dwGetStr(joStyleName,'lastsort');
        sOrderName  := dwGetStr(joStyleName,'ordername');
        sOrderDir   := dwGetStr(joStyleName,'orderdir');
        iOrderType  := dwGetInt(joStyleName,'ordertype');   //排序字段的数据类型
    end;

    //增加Aprimary， 防止没有APrimary字段时, where时报错
    if Pos(LowerCase(sPrimary+','),LowerCase(sFields+',')) <= 0 then begin
        sFields := sFields +','+sPrimary;
    end;

    //
    sSQL    := 'SELECT '+sFields+' FROM '+sTable + sWhere;
    if iPageNo = 0 then begin   //处理第一页
        //
        if sOrderName = '' then begin
            sSQL    := sSQL + ' ORDER BY '+sPrimary;
        end else begin
            sSQL    := sSQL + ' ORDER BY '+sOrderName+','+sPrimary;
        end;
        ...
        //数据正向显示
        bDir    := True;
    end else if iPageNo + 1 = Ceil(iRecCount/iPageSize) then begin   //处理最后一页
        iPageSize   := iRecCount - iPageSize * (Ceil(iRecCount/iPageSize)-1);

        //
        if sOrderName = '' then begin
            sSQL    := sSQL + ' ORDER BY '+sPrimary+' DESC';
        end else begin
            sSQL    := sSQL + ' ORDER BY '+sOrderName+' DESC,'+sPrimary+' DESC';
        end;
        ...
        if iDiff>0 then begin
            ...
            //增加链表分页的where
            if sOrderName = '' then begin
                sSQL    := sSQL + 'AND ('+sPrimary+'>'+IntToStr(iLastPri)+')';
            end else begin
                //区分数字和字符串型
                case TFieldType(iOrderType) of
                    ftSmallint, ftInteger, ftWord, ftFloat, ftCurrency, ftBCD, ftLongWord, ftShortint, ftByte, ftExtended, ftSingle: begin     //int
                        sSQL    := sSQL + 'AND (('+sOrderName+'>'+sLastSort+') OR (('+sOrderName+'='+sLastSort+') AND ('+sPrimary+'>'+IntToStr(iLastPri)+'))';
                    end;
                else
                        sSQL    := sSQL + 'AND (('+sOrderName+'>'''+sLastSort+''') OR (('+sOrderName+'='''+sLastSort+''') AND ('+sPrimary+'>'+IntToStr(iLastPri)+'))';
                end;
            ...
            //设置分页 跳过记录数
            AQuery.FetchOptions.RecsSkip    := (iDiff-1)*iPageSize;
            ...
            //增加链表分页的where
            if sOrderName = '' then begin
                sSQL    := sSQL + 'AND ('+sPrimary+'<'+IntToStr(iFirstPri)+')';
            end else begin
                //区分数字和字符串型
                case TFieldType(iOrderType) of
                    ftSmallint, ftInteger, ftWord, ftFloat, ftCurrency, ftBCD, ftLongWord, ftShortint, ftByte, ftExtended, ftSingle: begin     //int
                        sSQL    := sSQL + 'AND (('+sOrderName+'<'+sFirstSort+') OR (('+sOrderName+'='+sFirstSort+') AND ('+sPrimary+'<'+IntToStr(iFirstPri)+'))';
                    end;
                else
                        sSQL    := sSQL + 'AND (('+sOrderName+'<'''+sLastSort+''') OR (('+sOrderName+'='''+sFirstSort+''') AND ('+sPrimary+'<'+IntToStr(iFirstPri)+'))';
                end;
            end;
            ...
            //设置分页
            AQuery.FetchOptions.RecsSkip    := (abs(iDiff)-1)*iPageSize; //跳过记录数
    ...
    AQuery.FetchOptions.RecsMax     := iPageSize;           //返回记录数
    AQuery.Open;
    ...
    if AQuery.IsEmpty then begin
        ...
        if bDir then begin
            ...
            //<保存当前查询的第一条消息
            joStyleName.oldpageno       := iPageNo;
            joStyleName.firstprimary    := AQuery.FieldByName(sPrimary).AsInteger;
            if sOrderName = '' then begin
                joStyleName.firstsort   := '';
            end else begin
                joStyleName.firstsort   := AQuery.FieldByName(sOrderName).AsString;
            end;
            //>
            ...
            for iRow := 1 to ASG.RowCount-1 do begin
                    ...
                    case iMode of
                        0,3 : begin
                            if (joField.type = 'index') then begin
                                ...
                                ASG.Cells[iCol,iRow]        := cpGetFieldValue(AQuery.Fields[Max(0,iFieldId)],joField);
                            end;
                        end;
                    else
                    end;
                ...
                if AQuery.Eof then begin
                    //保存当前分页的末记录的相关信息
                    joStyleName.lastprimary     := AQuery.FieldByName(sPrimary).AsInteger;
                    if sOrderName = '' then begin
                        joStyleName.lastsort    := '';
                    end else begin
                        joStyleName.lastsort    := AQuery.FieldByName(sOrderName).AsString;
                    end;
                    //
                    break;
                end;
...
procedure cpCreateField(APanel:TPanel;AWidth:Integer;ASuffix:String;AConfig,AField:Variant;AP_Content:TFlowPanel);
type
...
begin
    ...
    with oPEr do begin
        ...
        Font.Color  := $323232;
        Color       := clWhite;
        Height      := Min(oForm.Height - 200,dwGetInt(joConfig,'editheight',500));
    ...
    for iField := 0 to joConfig.fields._Count-1 do begin
        ...
        //创建控件
        iMode   := dwGetInt(joField,'view',0);  //显示模式，0：全部显示，1：仅新增/编辑时显示，2：全不显示, 3:仅表格显示
        case iMode of
            0,1 : begin
                cpCreateField(
                        APanel,
...
begin
    try
        ...
        joDBConfig.primarykey   := joConfig.primarykey;     //主键,应为自增型int
