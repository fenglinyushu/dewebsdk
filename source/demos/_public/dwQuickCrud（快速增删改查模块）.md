...
- 更改字段的类型，有string, auto, integer, combo, dbcombo, money, date, time, datetime, memo等，默认为string
...
| combopair | 列表选择类型 | 具体选项由当前节点的list二维数组决定,显示为二维数组的元素0，保存为二维数据组的元素1 |
...
| height | 新增或编辑时高度  | "height":80 |仅type为memo时有效 |
...
2023-11-28 
	...
	Y 为事件时机。
		...
		4表示post后
		5表示数据更新后
	Accept 为中止变量。默认为True,如果设置为False, 则提示操作中止,执行dwMessage('terminate by user','error',self);
	其中：提示语可能通过配置文件的temenate设置

2023-11-27 
    1. 增加了type为combopair时，增加"键-值"对；
	2. 增加了dwCrudDestroy函数，可以清除Crud对象，以便二次Crud
...
2023-08-10
    1. 解决了“重置”未清空的问题；
    2. 通过判断 B_Query.tag 是否为1 ， 确定是否当前查询条件生效
