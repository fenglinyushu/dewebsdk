...
### 2025-11-04
1. 从原dwCrudpanel改造, 创建本单元
2. 使用了 name 代替原来的 table, 而table用于更新
*)
...
//一键生成CRUD模块
function csInit(APanel:TPanel;AConnection:TFDConnection;const AMobile:Boolean=False;const AReserved:String=''):Integer;

//添加存储过程的参数
function csAddParam(APanel:TPanel;AName:String;AValue:Variant):Integer;

//删除存储过程的参数
function csDeleteParam(APanel:TPanel;AName:String):Integer;

//清空存储过程的参数
function csClearParams(APanel:TPanel):Integer;

//取当前配置，可能与原来的hint有所区别，主要是增加了默认值和自动生成的配置
function  csGetConfig(APanel:TPanel):Variant;

//销毁dwCrud，以便二次创建
function  csDestroy(APanel:TPanel):Integer;

//更新数据
procedure csUpdate(APanel:TPanel;const AWhere:String = '';const AInited:Boolean=True);

//取CrudPanel模块的 FDQuery 控件。 未找到返回空值
function  csGetFDQuery(APanel:TPanel):TFDStoredProc;
function  csGetFDQueryTemp(APanel:TPanel):TFDQuery;

//取CrudPanel模块的 StringGrid 控件
function  csGetStringGrid(APanel:TPanel):TStringGrid;

//取得对应序号的编辑控件, 一般为TEdit/TComboBox/TDateTimePicker
function csGetEditComponent(APanel:TPanel;AIndex:Integer):TComponent;

//取CrudPanel模块的SQL语句
function  csGetSQL(APanel:TPanel):String;

//取CrudPanel模块的SQL语句中的where(返回值不包含where)
function  csGetSQLWhere(APanel:TPanel):String;

//在CrudPanel的按钮栏中插入控件(一般用于插入自定义的按钮)
function  csAddInButtons(APanel:TPanel;ACtrl:TWinControl):Integer;

//在CrudPanel的智能查询栏中插入控件
function  csAddInSmart(APanel:TPanel;ACtrl:TWinControl):Integer;

//设置where, AWhere 类似 'Id>20' , 直接更新APanel.Hint
function  csSetWhere(APanel:TPanel;AWhere:String):Integer;

//设置name的value
function  csSetNameValue(APanel:TPanel;AName:String;AValue:Variant):Integer;

//设置功能按钮标题显示样式	整数	1	0:全不显示,1:全显示,2:居左按钮显标题，居右按钮不显示
function  csSetButtonCaption(APanel:TPanel;AValue:Integer):Integer;

//设置field的值,通过name,
//如 csSetFieldByName(Pn1,'qCreatorId','default',9);
//即 把Pn1的Hint的json对象中fields中的name为qCreatorId的字段JSON对象中的default属性设置为9
function  csSetFieldByName(APanel:TPanel;AName,AAttr:String;AValue:Variant):Integer;

//设置额外的where, AWhere 类似 'Id>20' , 保存在PQm面板(也就是EKw的容器面板)中
function  csSetExtraWhere(APanel:TPanel;AWhere:String):Integer;

//取上述值
function  csGetExtraWhere(APanel:TPanel):String;

//设置为移动端模式，调整相关控件
function  csSetMobile(APanel:TPanel):Integer;

//设置"增删改查导入导出"等的Enabled , AName分别为new,delete,edit,query,import,export
function  csSetControlEnabled(APanel:TPanel;AName:string;AEnabled:Boolean):Integer;

//设置简洁模式，智能模糊查询模式下，将按钮和查询放到一排
function  csSetOneLine(APanel:TPanel;const AMargin:Integer=10):Integer;

//控制编辑时各编辑控件的值, 其中: AName 为字段json的name
function  csSetEditFieldValue(APanel:TPanel;AName,AValue:String):Integer;               //设置值
function  csGetEditFieldValue(APanel:TPanel;AName:String;var AValue:String):Integer;    //取编辑控件值
function  csSetEditFieldReadonly(APanel:TPanel;AName:String;AReadOnly:Boolean):Integer; //设置编辑控件只读

//-----设置表格单元格的各种样式
//设置单元格背景色
function  csSetCellBackColor(APanel:TPanel;ACol,ARow:integer;AColor:TColor):Integer;
//设置单元格字体颜色
function  csSetCellFontColor(APanel:TPanel;ACol,ARow:integer;AColor:TColor):Integer;
//设置单元格字体大小
function  csSetCellFontSize( APanel:TPanel;ACol,ARow:integer;ASize:Integer):Integer;
//设置单元格字体粗体
function  csSetCellFontBold(APanel:TPanel;ACol,ARow:integer;ABool:Boolean):Integer;
//设置单元格字体斜体
function  csSetCellFontItalic(APanel:TPanel;ACol,ARow:integer;ABool:Boolean):Integer;
//设置单元格字体下划线
function  csSetCellFontUnderline(APanel:TPanel;ACol,ARow:integer;ABool:Boolean):Integer;
//设置单元格字体删除线
function  csSetCellFontStrikeout(APanel:TPanel;ACol,ARow:integer;ABool:Boolean):Integer;
//设置单元格显示/隐藏
function  csSetCellVisible(APanel:TPanel;ACol,ARow:integer;AVisible:Boolean):Integer;
//设置单元格内的按钮显示/隐藏
function  csSetCellButtonVisible(APanel:TPanel;ACol,ARow,AIndex:integer;AVisible:Boolean):Integer;
//设置单元格字的按钮禁用
function  csSetCellButtonEnabled(APanel:TPanel;ACol,ARow,AIndex:integer;AEnabled:Boolean):Integer;
//设置单元格-合并
function  csSetCellMerge(APanel:TPanel;ACol,ARow,AColSpan, ARowSpan:integer):Integer;
//设置单元格innerHTML
function  csSetCellHTML(APanel:TPanel;ACol,ARow:integer;AHTML:String):Integer;
//设置表格的指定列显示/隐藏
function  csSetColVisible(APanel:TPanel;ACol:integer;AVisible:Boolean):Integer;
...
function csDbToTreeListJson(AQuery:TFDQuery;ATable,AData,AView,ALink:String;ACount:Integer):Variant;

//将函数cpDbToTreeListJson生成的JSON转化为TreeView
function csTreeListJsonToTV(AList:Variant;ATV:TTreeView):Integer;

//删除当前表的一部分记录
function csDeleteRecords(APanel:TPanel;AWhere:String):Integer;

const
    //整型类型集合
    csstInteger : Set of TFieldType = [ftSmallint, ftInteger, ftWord, ftAutoInc,ftLargeint, ftLongWord, ftShortint, ftByte];
    //浮点型类型集合
    csstFloat   : set of TFieldType = [ftFloat, ftCurrency, ftBCD, ftExtended, ftSingle];
    //日期时间类型集合
    csstDateTime   : set of TFieldType = [ftDate, ftTime, ftDateTime, ftTimeStamp,ftOraTimeStamp,ftTimeStampOffset];
...
begin
    ...
    //
    joConfig    := csGetConfig(APanel);
    if not joConfig.Exists('params') then begin
...
begin
    ...
    try
        ...
        if joConfig.Exists('slave') then begin
            ...
            for iSlave := 0 to joConfig.slave._Count - 1 do begin
                ...
                if oPanelSlave <> nil then begin
                    //取得从表配置 json
                    joConfigS   := csGetConfig(oPanelSlave);

                    //递归删除记录
                    csDeleteRecords(oPanelSlave, joSlave.slavefield+' in (SELECT '+joSlave.masterfield+' FROM '+joConfig.name+' WHERE '+AWhere+')');
        ...
        //
        oFDTemp := csGetFDQueryTemp(APanel);
        oFDTemp.Connection.ExecSQL('DELETE FROM '+joConfig.table+' WHERE '+AWhere);

    except
        dwMessage('except when csDeleteRecords : '+IntTostr(Result),'error',oForm);
        Exit;
    end;
end;
...
function  csSetEditFieldValue(APanel:TPanel;AName,AValue:String):Integer;
var
    oForm       : TForm;
    oComp       : TComponent;
    //
...
begin
    ...
    try
        ...
        if iField = -1 then begin
            //dwMessage('error when csSetEditFieldValue','error',oForm);
            Exit;
        end;
        Dec(Result);
        ...
        if sType = 'auto' then begin                 //----- 1 -----
            ...
            if TryStrToDate(AValue, dtValue) then begin
                ...
                dwMessage('except when csSetEditFieldValue! not date format : '+AValue,'error',oForm);
                Exit;
            end;
            ...
            if TryStrToDateTime(AValue, dtValue) then begin
                ...
                dwMessage('except when csSetEditFieldValue! not datetime format : '+AValue,'error',oForm);
                Exit;
            end;
            ...
            if TryStrToFloat(AValue, fValue) then begin
                ...
                dwMessage('except when csSetEditFieldValue! not float format : '+AValue,'error',oForm);
                Exit;
            end;
            ...
            if TryStrToInt(AValue, iValue) then begin
                ...
                dwMessage('except when csSetEditFieldValue! not integer format : '+AValue,'error',oForm);
                Exit;
            end;
            ...
            if TryStrToFloat(AValue, fValue) then begin
                ...
                dwMessage('except when csSetEditFieldValue! not meney format : '+AValue,'error',oForm);
                Exit;
            end;
            ...
            if TryStrToTime(AValue, dtValue) then begin
                ...
                dwMessage('except when csSetEditFieldValue! not time format : '+AValue,'error',oForm);
                Exit;
            end;
        ...
        dwMessage('except when csSetEditFieldValue : '+IntTostr(Result),'error',oForm);
        Exit;
    end;
end;

function  csSetEditFieldReadonly(APanel:TPanel;AName:String;AReadOnly:Boolean):Integer;
var
    oForm       : TForm;
    oComp       : TComponent;
    //
...
begin
    ...
    try
        ...
        dwMessage('except when csSetEditFieldReadonly : '+IntTostr(Result),'error',oForm);
        Exit;
    end;
end;

function  csGetEditFieldValue(APanel:TPanel;AName:String;var AValue:String):Integer;
var
    oForm       : TForm;
    oComp       : TComponent;
    //
...
begin
    ...
    try
        ...
        if iField = -1 then begin
            //dwMessage('error when csGetEditFieldValue','error',oForm);
            Exit;
        end;
        Dec(Result);
        ...
        dwMessage('except when csGetEditFieldValue : '+IntTostr(Result),'error',oForm);
        Exit;
    end;
end;
...
function  csSetCellGetCell(APanel:TPanel;ACol,ARow:integer):string;
var
    oForm       : TForm;
    oSGD        : TStringGrid;
    sPrefix     : String;
...
function csGetMainForm(APanel:TPanel):TForm;    //取得主窗体. 而不是当前窗体
begin
    //取得主Form
...
var
    sJS     : String;
begin
    sJS     := csSetCellGetCell(APanel,ACol,ARow)+'.style.setProperty(''background'', '''+dwColorAlpha(AColor)+''', ''important'');';
    //sJS     :=
    //        'setTimeout(() => {'#13#10
    //        +sJS
    //        +#13#10'}, 1);';
    dwRunJSTimeout(sJS,csGetMainForm(APanel));
end;
...
var
    sJS     : String;
begin
    sJS     := csSetCellGetCell(APanel,ACol,ARow)+'.style.color = '''+dwColorAlpha(AColor)+''';';
    //sJS     :=
    //        'setTimeout(() => {'#13#10
    //        +sJS
    //        +#13#10'}, 1);';
...
function  csSetCellFontSize( APanel:TPanel;ACol,ARow:integer;ASize:Integer):Integer;
begin
    dwRunJSTimeout(csSetCellGetCell(APanel,ACol,ARow)+'.style.fontSize = '''+IntToStr(ASize)+'px'';',csGetMainForm(APanel));
end;

function  csSetCellFontBold(APanel:TPanel;ACol,ARow:integer;ABool:Boolean):Integer;
begin
    dwRunJSTimeout(csSetCellGetCell(APanel,ACol,ARow)+'.style.fontWeight = '''+dwIIF(ABool,'bold','normal')+''';',csGetMainForm(APanel));
end;

function  csSetCellFontItalic(APanel:TPanel;ACol,ARow:integer;ABool:Boolean):Integer;
begin
    dwRunJSTimeout(csSetCellGetCell(APanel,ACol,ARow)+'.style.fontStyle = '''+dwIIF(ABool,'italic','normal')+''';',csGetMainForm(APanel));
end;

function  csSetCellFontUnderline(APanel:TPanel;ACol,ARow:integer;ABool:Boolean):Integer;
begin
    dwRunJSTimeout(csSetCellGetCell(APanel,ACol,ARow)+'.style.textDecoration = '''+dwIIF(ABool,'underline','normal')+''';',csGetMainForm(APanel));
end;

function  csSetCellFontStrikeout(APanel:TPanel;ACol,ARow:integer;ABool:Boolean):Integer;
begin
    dwRunJSTimeout(csSetCellGetCell(APanel,ACol,ARow)+'.style.textDecoration = '''+dwIIF(ABool,'line-through','normal')+''';',csGetMainForm(APanel));
end;

function  csSetCellVisible(APanel:TPanel;ACol,ARow:integer;AVisible:Boolean):Integer;
begin
    dwRunJSTimeout(csSetCellGetCell(APanel,ACol,ARow)+'.style.visibility  = '''+dwIIF(AVisible,'visible','hidden')+''';',csGetMainForm(APanel));
end;

//取得CrudPanel对应的表格的相应单元格的对应button的dom字符串,为设置各项样式做准备
function  csSetCellGetButton(APanel:TPanel;ACol,ARow,AIndex:integer):string;
begin
    Result  := csSetCellGetCell(APanel,ACol,ARow)+'.childNodes[0].children['+IntToStr(AIndex)+']';
end;

function  csSetCellButtonVisible(APanel:TPanel;ACol,ARow,AIndex:integer;AVisible:Boolean):Integer;
begin
    dwRunJSTimeout(csSetCellGetButton(APanel,ACol,ARow,AIndex)+'.style.display  = '''+dwIIF(AVisible,'block','none')+''';',csGetMainForm(APanel));
end;

function  csSetCellButtonEnabled(APanel:TPanel;ACol,ARow,AIndex:integer;AEnabled:Boolean):Integer;
begin
    dwRunJSTimeout(csSetCellGetButton(APanel,ACol,ARow,AIndex)+'.disabled = '+dwIIF(AEnabled,'false','true')+';',csGetMainForm(APanel));
end;
...
var
    sJS     : String;
begin
    sJS     := 'var ocell='+csSetCellGetCell(APanel,ACol,ARow)+';'
...
function  csSetCellHTML(APanel:TPanel;ACol,ARow:integer;AHTML:String):Integer;
begin
    dwRunJSTimeout(csSetCellGetCell(APanel,ACol,ARow)+'.innerHTML = '''+AHTML+''';',csGetMainForm(APanel));
end;
...
(*  暂不可用
    ...
    for iRow := 0 to oSGD.RowCount - 1 do begin
        if not AVisible then begin
            dwRunJS(csSetCellGetCell(APanel,ACol,iRow)+'.style.width  = ''0'';',TForm(APanel.Owner));
        end;
        dwRunJS(csSetCellGetCell(APanel,ACol,iRow)+'.style.display  = '''+dwIIF(AVisible,'block','none')+''';',TForm(APanel.Owner));
        //dwRunJS(csSetCellGetCell(APanel,ACol,iRow)+'.style.lineHeight = '''+IntToStr(oSGD.DefaultRowHeight)+'px'';',TForm(APanel.Owner));
    end;
*)
end;

//设置当前查询模式. 0:无查询, 1:搜索框智能模糊查询 2:多字段精准/模糊查询
function csSetQueryMode(APanel:TPanel;AMode:Integer;AInited:Boolean):Integer;
var
    oForm       : TForm;
...
//AViewField 显示在表格和查询中的内容字段名
//ALink为多级之间的连接字符
function csDbToTreeList(AQuery:TFDQuery;ATable,AData,AView,ALink:String;ACount:Integer):String;
var
    //
    joRes       : Variant;
begin
    try
        //先生成JSON
        joRes   := csDbToTreeListJSON(AQuery,ATable,AData,AView,ALink,ACount);
...
function csGetCrudPanel(ACtrl:TControl):TPanel;
var
    oCtrl   : TControl;
begin
    /////////////////////////
    ///原理是查找当前控件的父控件，如果是TPanel且HelpContext为31029，则停止
    /////////////////////////

    Result  := nil;
    if (ACtrl <> nil) and (ACtrl.Parent<>nil) then begin
        if (ACtrl.ClassType = TPanel) and (ACtrl.HelpContext = 31029) then begin
            Result  := TPanel(ACtrl);
        end else begin
            oCtrl   := ACtrl;
            while True do begin
                ...
                if (oCtrl.ClassType = TPanel) and (oCtrl.HelpContext = 31029) then begin
                    Result  := TPanel(oCtrl);
                    break;
                end;
            end;
...
begin
    try
        ...
        if not Result.Exists('name') then begin        //默认表名
            Result.name := '';
        end;
...
Procedure csLinkageChange(Sender: TObject);    //带Linkage属性的联动事件
var
    oCombo      : TComboBox;
    oComboLink  : TComboBox;    //被控的字段
    oCrudPanel  : TPanel;
    oForm       : TForm;
    oFQTemp     : TFDStoredProc;
    //
    sPrefix     : String;
    sName       : string;
...
begin
    ...
    //取得源crud panel
    oCrudPanel  := csGetCrudPanel(TControl(Sender));

    //取得当前配置
    joConfig    := csGetConfig(oCrudPanel);
    ...
    //取得目标 comboBox
    oFQTemp     := TFDStoredProc(oCrudPanel.FindComponent(sPrefix+'FQTemp'));
    ...
    for iItem := 0 to joField.linkages._Count - 1 do begin
        ...
        //替换{aaa}
        if TFieldType(joField.datatype) in csstInteger then begin
...
Procedure csFieldChange(Self: TObject; Sender: TObject);
var
    oCrudPanel  : TPanel;
    bAccept     : boolean;
    //
...
begin
    ...
    if joField.Exists('linkages') then begin
        csLinkageChange(Sender);
    end;

end;

Procedure csFieldEnter(Self: TObject; Sender: TObject);
var
    oCrudPanel  : TPanel;
	bAccept		: boolean;
begin
...
Procedure csFieldExit(Self: TObject; Sender: TObject);
var
    oCrudPanel  : TPanel;
	bAccept		: boolean;
begin
...
//根据列号，求JSON序号
function  csGetJsonIdFromColId(AConfig:Variant;ACol:Integer):Integer;
var
    iItem       : Integer;
    iCurCol     : Integer;
    joField     : Variant;
...
function csGetCaptions(AFields:Variant;AVisbileOnly:Boolean):String;
var
    iField      : Integer;
    joField     : variant;
    joRes       : Variant;
...
function csGetIndexs(AFields:Variant;AVisbileOnly:Boolean):String;
var
    iField      : Integer;
    joField     : variant;
    joRes       : Variant;
...
function csGetFields(AFields:Variant;AVisibleOnly:Boolean):String;
var
    iField      : Integer;
    joField     : variant;
...
function csGetSelection(APanel:TPanel):String;
var
    sPrefix     : String;
    oForm       : TForm;
    oSG         : TStringGrid;
...
function csGetTreeListJson(AList:String):Variant;
begin
...
function csGetTreeListView(AList:Variant;AData:String):String;
var
    iItem   : Integer;
begin
    Result  := '';
        ...
        for iItem := 0 to AList._Count - 1 do begin
            if AList._(iItem).id = AData then begin
                ...
                if AList._(iItem).Exists('children') then begin
                    Result  := csGetTreeListView(AList._(iItem).children,AData);
...
function csGetTreeListData(AList:Variant;AView:String):String;
var
    iItem   : Integer;
begin
    Result  := '';
    ...
    if AList._Count >0 then begin
        for iItem := 0 to AList._Count - 1 do begin
            if AList._(iItem).label = AView then begin
                ...
                if AList._(iItem).Exists('children') then begin
                    Result  := csGetTreeListData(AList._(iItem).children,AView);
                    if Result <> '' then begin
                        break;
                    end;
                end;
...
begin
    try
        ...
        joInfo.sqltext  := AQuery.StoredProcName;//  .SQL.Text;
        ...
        //AQuery.SQL.Text := 'SELECT '+sFields+' FROM '+String(+joConfig.name )+' WHERE (1=0)';
        AQuery.StoredProcName   := joConfig.name;
...
//取数据字段的值
function csGetFieldValue(AField:TField;AConfig:Variant):String;
var
    iTemp       : Integer;
    sTemp       : string;
    joField     : Variant;
...
begin
    ...
    if AConfig.type = 'auto' then begin                     //----- 1 -----
        ...
        if AField.AsString <> '' then begin
            //将tree的list转化为JSON
            joList  := csGetTreeListJson(AConfig.list);
            //查找对应的显示值
            Result  := csGetTreeListView(joList,AField.AsString);
        end;
    ...
    if (Pos('{{',Result)>0) and (Pos('}}',Result)>0) then begin
        with TFDStoredProc(AField.Dataset) do begin
...
//根据表名、字段名，生成选项JSON数组
function csGetItems(
        AQuery:TFDStoredProc;       //对应的TFDQuery控件
        ATable:string;              //表名
        AField:string
        ):Variant;
var
...
begin
    try
        ...
        slRes.Sorted        := True;
        slRes.Duplicates    := dupIgnore;
        //
        AQuery.Close;
        AQuery.StoredProcName   := ATable;
        AQuery.Filtered         := False;
        AQuery.IndexFieldNames  := '';
        AQuery.Open;

        //
        while not AQuery.Eof do  begin
            slRes.Add(AQuery.FieldByName(AField).AsString);
            AQuery.Next;
        end;

        for iItem := 0 to slRes.Count - 1 do begin
            Result.Add(slRes[iItem]);
        end;
...
//计算字段列表中在表格中显示的数量
function csGetViewFieldCount(AFields:Variant):Integer;
var
    iItem   : Integer;
begin
    Result  := 0;
...
//群友"顺其自然"写的带关联查询的函数
function csGetWhereJoin(AFields:Variant;AKeyWord:string):string;
var
    iField      : Integer;
    iList       : Integer;
    joField     : variant;
        ...
        for iField := 0 to AFields._count-1 do begin
            ...
            if (sType = 'dbcombopair') then begin
                if joField.Exists('name') and joField.Exists('table') and joField.Exists('datafield') and joField.Exists('viewfield') then begin
                    Result:=dwIIF(Result='','',Result+' or ')+'('+joField.name+' IN (SELECT '+joField.datafield+' FROM '+joField.name+' where '+joField.viewfield+' like ''%'+AKeyWord+'%''))';
                end;
            end else if (sType = 'dbtreepair') then begin
                if joField.Exists('name') and joField.Exists('table') and joField.Exists('datafield') and joField.Exists('viewfield') then begin
                    Result:=dwIIF(Result='','',Result+' or ')+'('+joField.name+' IN '
                    +'(SELECT DISTINCT '+joField.datafield+' FROM '+joField.name+' d WHERE EXISTS (SELECT 1 FROM '+joField.name+' b WHERE b.'+joField.viewfield+' LIKE ''%'+AKeyWord+'%'' AND d.'+joField.datafield+' LIKE b.'+joField.datafield+' + ''%'' AND (LEN(d.'+joField.datafield+') - LEN(b.'+joField.datafield+')) % 2 = 0)));';
                end;
            end else if (sType = 'combopair') then begin     //----- 5 -----
...
begin
    if Trim(AKeyword)='' then begin
        ...
        for iKey := 0 to High(sKeywords) do begin
            ...
            sTmp    := csGetWhereJoin(AjoFields,sKeywords[iKey]);
            if sTmp<>'' then begin
...

{
        ...
        //sTmp1    := '';
        //for iKey := 0 to high(sKeywords) do begin
        //    sTmp    := csGetWhereJoin(AFields,sKeywords[iKey]);
...
//注意:必须有id自增字段
procedure csGetPageData(
        APanel:TPanel;
        AQuery:TFDStoredProc;        //对应的ADOQuery控件
        ATable:string;          //表名
        AFields:string;         //字段列表  = '*'或'Name,Age,job,title'
...
begin
    ...
    AQuery.Close;
    AQuery.StoredProcName   := ATable;//.SQL.Text := 'SELECT Count(*) AS AA FROM '+ATable+' '+AWhere;
    AQuery.Filtered := True;
    AQuery.Filter   := AWhere;
    AQuery.Open;

    //记录总数
    ARecordCount    := AQuery.RecordCount;//.Fields[0].AsInteger;
...
function csGetPageSQL(APanel:TPanel;ADatabase,ATable,AFields,AWhere,APrimary,ASort:String;
...
function csGetDataToGrid(
        AQuery      : TFDStoredProc;
        ASG         : TStringGrid;
        ATrackBar   : TTrackBar;
...
begin
    ...
    //取得源crud panel
    oCrudPanel  := csGetCrudPanel(TControl(ASG));
    ...
    //
    AQuery.Close;
    AQuery.StoredProcName   := sTable;
    AQuery.Prepare;
    AQuery.Filtered         := True;
    AQuery.Filter           := sWhere;
    AQuery.IndexFieldNames  := sOrder;
    AQuery.Open;

    //记录总数
    iRecCount   := AQuery.RecordCount;//.Fields[0].AsInteger;

    //如果超出最大页数,则为最大页数
    iPageNo     := Min(iPageNo,Ceil(iRecCount / iPageSize));
    ...
    if AQuery.IsEmpty then begin
        ...
        //先跳转到指定记录
        AQuery.RecNo    := iPageNo * iPageSize + 1;

        //
        for iRow := 1 to ASG.RowCount-1 do begin
            for iCol := 0 to ASG.ColCount - 1 do begin
                //取得当前列对应的JSON对象index
                iJsonID := csGetJsonIdFromColId(AConfig,iCol);
                ...
                case iMode of
                    0,3 : begin
                        if (joField.type = 'index') then begin
                            ...
                            //得到当前表格的值
                            //sValue  := csGetFieldValue(AQuery.Fields[Max(0,iFieldId)],joField);
                            sValue  := csGetFieldValue(AQuery.FieldByName(joField.name),joField);
...
//根据显示的值, 取得数据表存储的值
function csGetPairData(AField:Variant;AValue:String):String;
var
    iItem   : Integer;
    joList  : Variant;
begin
    if (AField.type = 'combopair') or (AField.type = 'dbcombopair') then begin
        ...
        //
        joList  := csGetTreeListJson(AField.list);

        //
        Result  := csGetTreeListData(joList,AValue);
...
begin
    ...
    //求当前模块的 FDQuery
    Result  := TFDStoredProc(APanel.FindComponent(sPrefix+'FQMain'));
end;
...
begin
    ...
    oFDQuery    := TFDStoredProc(APanel.FindComponent(sPrefix+'FQMain'));
    if oFDQuery<>nil then begin
        Result  := oFDQuery.StoredProcName;//  .SQL.Text;
    end;
end;
...
begin
    try
        ...
        //得到主表的字段名称列表
        sFields     := csGetFields(joConfig.Fields,joConfig.exportonlyvisible=0);

        //得到主表的字段caption列表,如：["姓名","性别","年龄","地址"]
        sTitles     := csGetCaptions(joConfig.Fields,joConfig.exportonlyvisible=0);

        //得到主表的字段index列表,如：[1,3,5,6]
        sIndexs     := csGetIndexs(joConfig.Fields,joConfig.exportonlyvisible=0);
        joIndex     := _json(sIndexs);

        //得到主表的WHERE
        sWhere      := csGetSQLWhere(oCrudPanel);

        //打开数据表
        oFQTemp.Close;
        //oFQTemp.SQL.Text := 'SELECT '+sFields+' FROM '+joConfig.name +' WHERE '+sWhere;
        oFQTemp.StoredProcName  := joConfig.name;
        oFQTemp.Filtered        := True;
        oFQTemp.Filter          := sWhere;
        oFQTemp.IndexFieldNames := '';
        //
                ...
                while not oFQTemp.Eof do begin     //循环遍历所有记录
                    for I := 0 to oFQTemp.FieldCount - 1 do  begin   //循环遍历所有字段
                        ...
                        if sType = 'index' then begin
                            ...
                            oWorkBook.Sheets[0].CellRef[_ColToStr(I), oFQTemp.RecNo].AsString := csGetFieldValue(oFQTemp.Fields[i],joField);
                        end;
...
begin
    try
        ...
        //取得各控件
        oButton := TButton(Sender);
        oQuery  := TFDStoredProc(oCrudPanel.FindComponent(sPrefix+'FQMain'));
...
begin
    ...
    //
    csSetQueryMode(oCrudPanel,oButton.Tag,True);
end;
...
var
    ...
    iRow        : Integer;
    bAccept     : Boolean;
    bPrimaryInt : boolean;  //Hint中指定的表的primary字段是否为int类型
    bLinkInt    : Boolean;  //主从表的关联字段是否为整型
    //
    joConfig    : variant;
...
begin
    ...
    //取得主表的primary字段是否为int, 备用
    bPrimaryInt := oQuery.FieldByName(joConfig.primary).DataType in csstInteger;
    ...
    //function csDeleteRecords(APanel:TPanel;AWhere:String):Integer;
    //分2步: 1 生成 WHERE ; 2 调用 csDeleteRecords 删除记录
    ...
    if joSelect._Count = 0 then begin
        ...
        //主表跳转到指定的记录
        oQuery.RecNo    := oSGD.Row + dwGetInt(joConfig,'pagesize',10) * oTbP.Position;
        ...
        if joConfig.Exists('slave') then begin
            ...
            for iSlave := 0 to joConfig.slave._Count - 1 do begin
                ...
                //主从表的关联字段是否为整型, 备用
                bLinkInt    := oQuery.FieldByName(joSlave.masterfield).DataType in csstInteger;
                ...
                if oPanelSlave <> nil then begin
                    //删除从表中相关记录
                    if bLinkInt then begin
                        sWhere  := string(joSlave.slavefield)+' = '+IntToStr(oQuery.FieldByName(joSlave.masterfield).AsInteger);
                    end else begin
                        //sWhere  := joSlave.masterfield;
                        //sWhere  := oQuery.FieldByName(sWhere).AsString;
                        //sWhere  := String(joSlave.slavefield)+' = '''+sWhere+'''';
                        sWhere  := String(joSlave.slavefield)+' = '''+oQuery.FieldByName(joSlave.masterfield).AsString+'''';
                    end;
                    //
                    csDeleteRecords(oPanelSlave,sWhere);
        ...
        //删除当前记录
        if bPrimaryInt then begin
            oQuery.Connection.ExecSQL('DELETE FROM ' + joConfig.table + ' WHERE '+joConfig.primary + ' = ' + IntToStr(oQuery.FieldByName(joConfig.primary).AsInteger));
        end else begin
            oQuery.Connection.ExecSQL('DELETE FROM ' + joConfig.table + ' WHERE '+joConfig.primary + ' = ''' + oQuery.FieldByName(joConfig.primary).AsString + '''');
        end;
        ...
        for iSelect := joSelect._Count - 1 downto 0 do begin //反向
            ...
            //跳转到相关记录处
            oQuery.RecNo    := iRow  + dwGetInt(joConfig,'pagesize',10) * oTbP.Position;
            ...
            if joConfig.Exists('slave') then begin
                ...
                for iSlave := 0 to joConfig.slave._Count - 1 do begin
                    ...
                    if oPanelSlave <> nil then begin
                        ...
                        //删除从表中相关记录
                        if bLinkInt then begin
                            csDeleteRecords(oPanelSlave,String(joSlave.slavefield)+' = '+IntToStr(oQuery.FieldByName(joSlave.masterfield).AsInteger));
                        end else begin
                            csDeleteRecords(oPanelSlave,String(joSlave.slavefield)+' = '''+oQuery.FieldByName(joSlave.masterfield).AsString+'''');
                        end;
                    end;
                end;
            end;
...
Procedure ButtonDeletaCancelClick(Self: TObject; Sender: TObject); //button Delete Cancel
var
    oButton : TButton;
    oForm   : TForm;
    oPanel  : TPanel;
...
*)
    ...
    if joInfo <> unassigned then begin
        ...
        oQuery.StoredProcName   := joInfo.sqltext;
        oQuery.Filtered         := False;
        //oQuery.Filter           := sWhere;
        oQuery.IndexFieldNames  := '';
...
Procedure ButtonEditMaxClick(Self: TObject; Sender: TObject);     //button edit max
var
    oCrudPanel  : TPanel;
    oButton     : TButton;
    oForm       : TForm;
...
begin
    ...
    for iItem := 0 to joConfig.fields._Count-1 do begin  //此处需要避免多选列问题
        ...
        if oComp <> nil then begin
            ...
            sValue  := oQuery.FieldByName(joField.name).AsString;
            ...
            if joField.type = 'auto' then begin                     //----- 1 -----
                ...
                oDT         := TDateTimePicker(oComp);
                oDT.Kind    := dtkDate;
                if oQuery.FieldByName(joField.name).IsNull then begin
                    oDT.Date    := StrToDate('1899-12-30');
                end else begin
                    oDT.Date    := oQuery.FieldByName(joField.name).AsDateTime;
                end;
                ...
                oE          := TEdit(oComp);
                //joList      := csGetTreeListJson(joField.list);
                //oE.Text     := csGetTreeListView(joList,sValue);
                oE.Text     := sValue;
            end else if joField.type = 'float' then begin           //----- 12 -----
                oE          := TEdit(oComp);
                oE.Text     := sValue;
                ...
                oM          := TMemo(oComp);
                oM.Text     := oQuery.FieldByName(joField.name).AsString;//sValue;
...
var
    ...
    sPrimary    : String;   //当前表的主键, 主要用于"编辑"数据
    sSQL        : String;   //用于编辑/新增时的SQL
    //
    joConfig    : variant;
    joField     : Variant;
    joMaster    : Variant;
    joInfo      : Variant;
    joDatas     : Variant;  //用于更新数据, 形如:[["uName","string","张三"],["uAge","integer",36]]
    joData      : Variant;
    //
    sPrefix     : String;

    //
    procedure csSaveData;
    var
        iiTmp   : integer;
    begin
        try
            ...
            if joField.type = 'auto' then begin                     //----- 1 -----
                ...
                //oQuery.FieldByName(joField.name).AsBoolean := (oCB_Field.text = oCB_Field.Items[1]);
                joData  := _json('[]');
                joData.Add(joField.name);
                joData.Add(joField.type);
                joData.Add((oCB_Field.text = oCB_Field.Items[1]));
                joDatas.Add(joData);
                ...
                oCB_Field   := TComboBox(oComp);
                //oQuery.FieldByName(joField.name).AsString  := oCB_Field.text;
                joData  := _json('[]');
                joData.Add(joField.name);
                joData.Add(joField.type);
                joData.Add(oCB_Field.text);
                joDatas.Add(joData);

            end else if (joField.type = 'combopair') then begin     //----- 5 -----
                oCB_Field   := TComboBox(oComp);
                oQuery.FieldByName(joField.name).AsString  := '';
                if oCB_Field.Text <> '' then begin
                    for iiTmp := 0 to joField.list._Count - 1 do begin
                        if joField.list._(iiTmp)._(1) = oCB_Field.text then begin
                            //oQuery.FieldByName(joField.name).AsString  := joField.list._(iiTmp)._(0);
                            joData  := _json('[]');
                            joData.Add(joField.name);
                            joData.Add(joField.type);
                            joData.Add(joField.list._(iiTmp)._(0));
                            joDatas.Add(joData);
                            break;
                        end;
                    end;
                ...
                //oQuery.FieldByName(joField.name).AsDateTime    := oDT_Field.Date;
                joData  := _json('[]');
                joData.Add(joField.name);
                joData.Add(joField.type);
                joData.Add(Double(oDT_Field.Date));
                joDatas.Add(joData);

            end else if joField.type = 'datetime' then begin        //----- 7 -----
                oDT_Field   := TDateTimePicker(oComp);
                //oQuery.FieldByName(joField.name).AsDateTime    := oDT_Field.DateTime;
                joData  := _json('[]');
                joData.Add(joField.name);
                joData.Add(joField.type);
                joData.Add(Double(oDT_Field.DateTime));
                joDatas.Add(joData);
                ...
                //oQuery.FieldByName(joField.name).AsString := oE_Field.text;
                joData  := _json('[]');
                joData.Add(joField.name);
                joData.Add(joField.type);
                joData.Add(oE_Field.text);
                joDatas.Add(joData);
                ...
                oE_Field := TEdit(oComp);
                //oQuery.FieldByName(joField.name).AsFloat := StrToFloatDef(oE_Field.text,0);
                joData  := _json('[]');
                joData.Add(joField.name);
                joData.Add(joField.type);
                joData.Add(StrToFloatDef(oE_Field.text,0));
                joDatas.Add(joData);
                ...
                oE_Field := TEdit(oComp);
                //oQuery.FieldByName(joField.name).AsInteger := StrToIntDef(oE_Field.text,0);
                joData  := _json('[]');
                joData.Add(joField.name);
                joData.Add(joField.type);
                joData.Add(StrToIntDef(oE_Field.text,0));
                joDatas.Add(joData);
                ...
                //oQuery.FieldByName(joField.name).AsString := oM_Field.text;
                joData  := _json('[]');
                joData.Add(joField.name);
                joData.Add(joField.type);
                joData.Add(oM_Field.text);
                joDatas.Add(joData);
                ...
                oDT_Field   := TDateTimePicker(oComp);
                //oQuery.FieldByName(joField.name).AsDateTime    := oDT_Field.Time;
                joData  := _json('[]');
                joData.Add(joField.name);
                joData.Add(joField.type);
                joData.Add(Double(oDT_Field.Time));
                joDatas.Add(joData);
    ...
    try
        ...
        case oPEr.Tag of
            0 : begin   //编辑
                    ...
                    if oComp <> nil then begin
                        ...
                        if joField.Exists('unique') then begin
                            if joField.unique = 1 then begin
                                ...
                                then begin
                                    //oFQTemp.SQL.Text  := 'SELECT Count(*) FROM '+joConfig.name
                ...
                //<-----将修改保存到数据表, 类似原oQuery.post; 现采用sql语句来完成
                //oQuery.FetchOptions.RecsSkip  := -1;
                //oQuery.CachedUpdates    := True;
                //oQuery.Post;
                //oQuery.ApplyUpdates;
                if dwGetStr(joConfig,'primary')='' then begin
                    //
                    dwMessage('保存失败！需要在配置中定义主键 primary 属性！','error',oForm);

                end else begin
                    //创建编辑 SQL
                    sSQL    := 'UPDATE '+joConfig.table+' SET ';

                    //
                    for iField := 0 to joDatas._Count - 1 do begin
                        //
                        joData  := joDatas._(iField);

                        //
                        if jodata._(1) = 'auto' then begin                     //----- 1 -----

                        end else if jodata._(1) = 'boolean' then begin         //----- 2 -----
                            sSQL    := sSQL + jodata._(0) +' = ' + dwIIF(jodata._(2),'1','0') + ',';

                        end else if jodata._(1) = 'button' then begin          //----- 3 -----

                        end else if (jodata._(1) = 'combo') then begin         //----- 4 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if (jodata._(1) = 'combopair') then begin     //----- 5 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if jodata._(1) = 'date' then begin            //----- 6 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + FormatDateTime('YYYY-MM-DD',TDateTime(jodata._(2))) + '''' + ',';

                        end else if jodata._(1) = 'datetime' then begin        //----- 7 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + FormatDateTime('YYYY-MM-DD hh:mm:ss',TDateTime(jodata._(2))) + '''' + ',';

                        end else if (jodata._(1) = 'dbcombo') then begin       //----- 8 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if (jodata._(1) = 'dbcombopair') then begin   //----- 9 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if jodata._(1) = 'dbtree' then begin          //----- 10 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if jodata._(1) = 'dbtreepair' then begin      //----- 11 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if jodata._(1) = 'float' then begin           //----- 12 -----
                            sSQL    := sSQL + jodata._(0) +' = ' + FloatToStr(jodata._(2))  + ',';

                        end else if jodata._(1) = 'image' then begin           //----- 13 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if jodata._(1) = 'index' then begin           //----- 13A -----

                        end else if jodata._(1) = 'integer' then begin         //----- 14 -----
                            sSQL    := sSQL + jodata._(0) +' = ' + IntToStr(jodata._(2))  + ',';

                        end else if (jodata._(1) = 'linkage') then begin         //----------
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if (jodata._(1) = 'linkagepair') then begin     //----------
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if jodata._(1) = 'memo' then begin            //----- 15 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if jodata._(1) = 'money' then begin           //----- 16 -----
                            sSQL    := sSQL + jodata._(0) +' = ' + FloatToStr(jodata._(2))  + ',';

                        end else if jodata._(1) = 'string' then begin          //----- 17 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if jodata._(1) = 'time' then begin            //----- 18 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + FormatDateTime('hh:mm:ss',TDateTime(jodata._(2))) + '''' + ',';

                        end else if jodata._(1) = 'tree' then begin            //----- 19 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';

                        end else if jodata._(1) = 'treepair' then begin        //----- 20 -----
                            sSQL    := sSQL + jodata._(0) +' = ''' + jodata._(2) + '''' + ',';
                    ...
                    //增加基于 primary 字段的 where
                    sPrimary    := oQuery.FieldByName(dwGetStr(joConfig,'primary')).AsString;
                    if oQuery.FieldByName(dwGetStr(joConfig,'primary')).DataType in csstInteger then begin
                        sSQL    := sSQL + ' WHERE ' + dwGetStr(joConfig,'primary') + ' = ' + sPrimary;
                    end else begin
                        sSQL    := sSQL + ' WHERE ' + dwGetStr(joConfig,'primary') + ' = ''' + sPrimary+'''';
                    end;

                    //执行SQL
                    oQuery.Connection.ExecSQL(sSQL);
            ...
            100 : begin   //新增
                ...
                if joConfig.Exists('master') then begin
                    ...
                    if joMaster.Exists('panel') and joMaster.Exists('masterfield') and joMaster.Exists('slavefield') then begin
                        ...
                        if oMasterPanel <> nil then begin
                            //取得主表查询
                            oFDQuery    := csGetFDQuery(oMasterPanel);
                ...
                //创建编辑 SQL
                sSQL    := 'INSERT INTO '+joConfig.table+' (';
                ...
                for iField := 0 to joDatas._Count - 1 do begin
                    ...
                    if jodata._(1) = 'auto' then begin                     //----- 1 -----
                        ...
                        sSQL    := sSQL + dwIIF(jodata._(2),'1','0') + ',';

                    end else if jodata._(1) = 'button' then begin          //----- 3 -----

                    end else if (jodata._(1) = 'combo') then begin         //----- 4 -----
                        sSQL    := sSQL + '''' + jodata._(2) + '''' + ',';

                    end else if (jodata._(1) = 'combopair') then begin     //----- 5 -----
                        sSQL    := sSQL + '''' + jodata._(2) + '''' + ',';

                    end else if jodata._(1) = 'date' then begin            //----- 6 -----
                        sSQL    := sSQL + '''' + FormatDateTime('YYYY-MM-DD',TDateTime(jodata._(2))) + '''' + ',';

                    end else if jodata._(1) = 'datetime' then begin        //----- 7 -----
                        sSQL    := sSQL + '''' + FormatDateTime('YYYY-MM-DD hh:mm:ss',TDateTime(jodata._(2))) + '''' + ',';
                        ...
                        sSQL    := sSQL + FloatToStr(jodata._(2))  + ',';
                        ...
                        sSQL    := sSQL + '''' + FormatDateTime('hh:mm:ss',TDateTime(jodata._(2))) + '''' + ',';
                ...
                if oCKB.Checked then begin
                    ...
                    //自动新增
                    csAppend(oCrudPanel,oQuery,joConfig.fields, dwGetInt(joConfig,'batchkeep') = 1);
...
begin
    ...
    //更新字段值
    csAppend(oCrudPanel, oQuery, joConfig.fields,False);
...
begin
    ...
    oFDQuery    := TFDStoredProc(oCrudPanel.FindComponent(sPrefix+'FQMain'));
                    ...
                    if oSlavePanel.HelpContext = 31028 then begin
                        ...
                        if oPQm <> nil then begin
                            ...
                            if oFDQuery.IsEmpty then begin
                                ...
                                //根据是否为整型分别生成where字符串
                                if (oFDQuery.FieldByName(String(joSlave.masterfield)).DataType in csstInteger) then begin
                        ...
                        if oFDQuery.IsEmpty then begin
                            csUpdate(oSlavePanel,'1=0');
                        end else begin
                            //根据是否为整型分别生成where字符串
                            if (oFDQuery.FieldByName(String(joSlave.masterfield)).DataType in csstInteger) then begin
                                csUpdate(oSlavePanel,String(joSlave.slavefield)+'='+oFDQuery.FieldByName(String(joSlave.masterfield)).AsString);
                            end else begin
                                csUpdate(oSlavePanel,String(joSlave.slavefield)+'='''+oFDQuery.FieldByName(String(joSlave.masterfield)).AsString+'''');
                            end;
                        end;
...
begin
    ...
    //取得页码和每页记录数
    iPageNo     := oTbP.Position;
    iPageSize   := dwGetInt(joConfig,'pagesize',10);

    //更新数据表记录
    oFDQuery.RecNo    := iRow + iPageNo * iPageSize;
    oFDQuery.Tag      := oFDQuery.RecNo;    //保存位置备用
...
begin
    ...
    //取得字段序号
    iFieldId    := csGetJsonIdFromColId(joConfig,iCol);
...
begin
    ...
    if joValue.type = 'sort' then begin
        ...
        //得到当前列信息
        joField     := joConfig.fields._(csGetJsonIdFromColId(joConfig,ACol));
        ...
        case ARow of
            0 : begin    //为降序,
                ...
                jostyleName.ordername   := joField.name + ':D';
        ...
        //根据列序号，取得JSON字段序号
        iField  := csGetJsonIdFromColId(joConfig,joValue.col);
        ...
        if joValue.data._Count = 0 then begin
            ...
            for iItem := 0 to joValue.data._Count - 1 do begin
                ...
                if          joField.type = 'boolean'        then begin  //----- 2 -----
                    ...
                    sData   := csGetPairData(joField,joValue.data._(iItem));
                end else if joField.type = 'dbcombopair'    then begin  //----- 9 -----
                    sData   := csGetPairData(joField,joValue.data._(iItem));
                end else if joField.type = 'dbtreepair'     then begin  //----- 11 -----
                    sData   := csGetPairData(joField,joValue.data._(iItem));
                ...
                if joField.Exists('datatype') then begin
                    ...
                    end else if TFieldType(joField.datatype) in csstFloat then begin
                        sFilter := sFilter  +' or ('+joField.name+'='+sData+')';
                    end else begin
                        //如果是pair系列,则需要转为
                        sFilter := sFilter  +' or ('+joField.name+'='''+sData+''')';
...
begin
    ...
    //更新数据
    csUpdate(csGetCrudPanel(TControl(Sender)),'');
end;


procedure csCreateConfirmPanel(APanel:TPanel);
var
    oForm       : TForm;
    oPDe        : TPanel;
    //用于指定事件
...
begin
    ...
    with oPDe do begin
        ...
        //取消事件
        tM.Code         := @ButtonDeletaCancelClick;
        tM.Data         := Pointer(325); // 随便取的数
        OnExit          := TNotifyEvent(tM);
    end;
end;

procedure csCreateField(APanel:TPanel;ASuffix:String;AConfig,AField:Variant;AP_Content:TFlowPanel);
type
...
begin
    ...
    if Pos(','+AField.type+',',',boolean,combo,combopair,date,datetime,dbcombo,dbcombopair,dbtree,dbtreepair,integer,memo,money,string,time,tree,treepair,')>0 then begin
        tM.Code         := @csFieldChange;
        tM.Data         := APanel.Owner;
        tM1.Code        := @csFieldEnter;
        tM1.Data        := APanel.Owner;
        tM2.Code        := @csFieldExit;
...
function  csCreateFieldQuery(APanel:TPanel;AConfig:Variant;AField:Variant;AIndex:Integer;ASuffix:String;AFlowPanel:TFlowPanel):Integer;
var
    oForm       : TForm;
    oPQF        : TPanel;
    oL_Query    : TLabel;
...
begin
    ...
    with oPQF do begin
        ...
        Height          := 45;
        Tag             := AIndex;  //保存字段的序号到tag中, 1000以上为
    ...
    if AField.type = 'auto' then begin                     //----- 1 -----
        ...
        with oCBQy do begin
            ...
            if AIndex < 1000 then begin
                ...
                //默认值
                ItemIndex   := dwIIFi(AField.value,1,0);
            end;
        end;
        ...
        if AIndex < 1000 then begin
            ...
            with oDT_Start do begin
                ...
                //检查min如果非数字, 则转化为日期; 否则按当前日期算差值
                Date        := StrToDateDef(AField.value,Now);
            end;
        end;
    end else if AField.type = 'datetime' then begin        //----- 7 -----
        if AIndex < 1000 then begin
            ...
            //更新标签
            oL_Query.Caption    := dwGetStr(AField,'caption',AField.name) + '>=';
            //创建查询起始日期
            oDT_Start   := TDateTimePicker.Create(oForm);
            with oDT_Start do begin
                Name            := sPrefix + 'DS'+ASuffix;
            ...
            with oL_Query do begin
                ...
                Align       := alLeft;
                Layout      := tlCenter;
                Caption     := dwGetStr(AField,'caption',AField.name) + '<=';
                Alignment   := taRightJustify;
                Width       := 80;
                //
                AlignWithMargins    := True;
            ...
            with oDT_Start do begin
                ...
                //检查min如果非数字, 则转化为日期; 否则按当前日期算差值
                DateTime        := StrToDateTimeDef(AField.value,Now);
            end;
        end;
...
procedure csCreateEditorPanel(APanel:TPanel);
var
    oForm       : TForm;
    oForm1      : TForm;
...
begin
    try
        ...
        //取得各控件备用
        oFDQuery    := TFDStoredProc(APanel.FindComponent(sPrefix+'FQMain'));    //主表数据库
        ...
        if ( oFQy <> nil ) and (oFQy.Visible) then begin    //oFQy为多字段查询框的容器FlowPanel Query
            ...
            if oFQy.Visible AND (oBQy.Tag = 1) then begin
                for iItem := 0 to oFQy.ControlCount-1 do begin
                    ...
                    //分两种情况处理. 1000以上为字段的条件;1000以上的为存储过程的参数
                    if iField < 1000 then begin
                        //取得当前字段对象
                        joField     := joConfig.fields._(iField);
                    ...
                    end else begin  //---- 1000以上为存储过程的参数
                        //取得当前字段对象
                        joField     := joConfig.params._(iField-1000);
                        sType       := dwGetStr(joField,'type','string');

                        //根据字段类型分类处理
                        //------------------ 7  csUpdate -----------------------------------------------------------------------
                        if sType = 'boolean' then begin         //----- 2 -----
                            oCBQy   := TComboBox(oPQF.Controls[1]);
                            joField.value := (oCBQy.ItemIndex = 1);
                            ...
                            oE_Query        := TEdit(oPQF.Controls[1]);
                            joField.value   := StrToFloatDef(Trim(oE_Query.Text),0);

                        end else if sType = 'integer' then begin         //----- 14 -----
                            oE_Query        := TEdit(oPQF.Controls[1]);
                            joField.value   := StrToIntDef(Trim(oE_Query.Text),0);

                        end else if sType = 'string' then begin          //----- 17 -----
                            oE_Query        := TEdit(oPQF.Controls[1]);
                            joField.value   := oE_Query.Text;
            ...
            //返回值为 ' WHERE (1=1) ' 或
            //         ' WHERE ((....) oR (...))'
            sWhere  := csGetWhere(joConfig.fields, oEKw.Text);

            //添加原配置JSON中自带的where
            if joConfig.where <> '' then begin
                Delete(sWhere,1,Length(' WHERE'));
                sWhere  := ' (('+joConfig.where+') and ' + sWhere+')';
            end;
        end;
        ...
        if joConfig.Exists('master') then begin
            ...
            if joMaster.Exists('panel') and joMaster.Exists('masterfield') and joMaster.Exists('slavefield') then begin
                ...
                if oMasterPanel <> nil then begin
                    //取得主表查询
                    oFQMaster   := csGetFDQuery(oMasterPanel);
                    ...
                    if oFQMaster <> nil then begin
                        if oFQMaster.IsEmpty then begin
                            ...
                            //根据主表更新从表
                            if (oFQMaster.FieldByName(String(joMaster.masterfield)).DataType in csstInteger) then begin
        ...
        sTmp    := Trim(csGetExtraWhere(APanel));
        if sTmp <> '' then begin
        ...
        //删除可能存在的 前置 WHERE
        sWhere  := Trim(sWhere);
        if LowerCase(copy(sWhere,1,6)) = 'where ' then begin
            Delete(sWhere,1,6);
        end;

        //移除无用 (1=1) where
        if sWhere = '(1=1)'  then begin
            sWhere  := '';
        end;

        //取得对应的参数设置
        iPageNo     := oTbP.Position;      //页码，从0始
        iPageSize   := joConfig.pagesize;    //每页行数
        sFields     := csGetFields(joConfig.fields,False);  //得到字段字符串列表

        //保存事件，并清空，以防止循环处理
        oEvent          := oTbP.OnChange;
        oTbP.OnChange   := nil;

        //保存原Recno
        iOldRecNo       := oFDQuery.RecNo;
        oFDQuery.DisableControls;
        ...
        //<打开存储过程
        oFDQuery.Close;
        oFDQuery.StoredProcName   := joConfig.name;
        oFDQuery.Filtered         := True;
        oFDQuery.Filter           := sWhere;
        ...
        if joConfig.Exists('params') then begin
            ...
            for iItem := 0 to joConfig.params._Count - 1 do begin
                //取得参数JSON对象
                joParam := joConfig.params._(iItem);

                //只处理包括name/value的节点
                if joParam.Exists('name') and joParam.Exists('value') then begin
                    //取得参数类型
                    sType   := dwGetStr(joParam,'type','');
                    sName   := String(joParam.name);
        ...
        //记录总数
        iRecCount   := oFDQuery.RecordCount;
        ...
        //设置分页控件值
        oTbP.Max       := iRecCount;   //所有页的记录之和
        oTbP.PageSize  := iPageSize;   //每页显示

        //清空原StringGrid数据记录(主要防止数据记录未写满页面显示错误)
        for iRow := 1 to oSGD.RowCount-1 do begin
            for iCol := 0 to oSGD.ColCount-1 do begin
                oSGD.Cells[iCol,iRow]    := '';
            end;
        end;
        oSGD.Row := 1;
        ...
        if oFDQuery.IsEmpty then begin
            //标记为空表
            oSGD.DoubleBuffered  := True;
        end else begin
            //标记为非空表
            oSGD.DoubleBuffered  := False;

            //先跳转到指定记录
            oFDQuery.RecNo    := iPageNo * iPageSize + 1;

            //
            for iRow := 1 to oSGD.RowCount-1 do begin
                for iCol := 0 to oSGD.ColCount - 1 do begin
                    //取得当前列对应的JSON对象index
                    iJsonID := csGetJsonIdFromColId(joConfig,iCol);

                    //异常检测
                    if (iJsonId < 0) or (iJsonId >= joConfig.fields._Count) then begin
                        Continue;
                    end;

                    //取得当前字段JSON对象
                    joField := joConfig.fields._(iJsonId);

                    //取得当前字段在整个数据表字段表中的序号index
                    iFieldId    := dwGetInt(joField,'fieldid',-1);

                    //异常检测
                    if (iFieldId < -1) or (iFieldId >= oFDQuery.Fields.Count) then begin
                        Continue;
                    end;
                        ...
                        0,3 : begin
                            if (joField.type = 'index') then begin
                                if (dwGetInt(joField,'full',1)=1) then begin
                                    ...
                                    //写入数据表格
                                    oSGD.Cells[iCol,iRow]    := IntToStr(iValue);
                                end else begin
                                    //对序号进行每页单独编号, 每页都从1开始编号
                                ...
                                //得到当前表格的值
                                //sValue  := csGetFieldValue(oFDQuery.Fields[Max(0,iFieldId)],joField);
                                sValue  := csGetFieldValue(oFDQuery.FieldByName(joField.name),joField);
        ...
        //恢复事件
        oTbP.Position  := iPageNo;
        oTbP.OnChange  := oEvent;

        //恢复原Recno
        oFDQuery.RecNo    := iOldRecNo;
        oFDQuery.EnableControls;

        //
        oFDQuery.RecNo  := 1 + dwGetInt(joConfig,'pagesize',10) * oTbP.Position;
        oSgD.Row        := 1;
...
var
    ...
    //
    oFQMain     : TFDStoredProc;    //主查询表
    oFQTemp     : TFDQuery;         //用于生成combo的临时性FDQuery
    oFieldType  : TFieldType;
    oFDMem      : TFDMemTable;      //临时表, 用于处理dbfilter
    ...
    //
    slFilter    : TStringList;  //用于处理dbfilter
    ...
    procedure csSetCaption(ALabel:TLabel);
    begin
...
begin
    ...
    try
        ...
        try
            ...
            APanel.HelpContext  := 31029;

            //取得form备用
            oForm   := TForm(APanel.Owner);

            //先检查，防止重复创建
            oFQMain       := TFDStoredProc(APanel.FindComponent(sPrefix+'FQMain'));
            ...
            //如果是临时表, 则自动禁止"增/删/改/导入"
            sText   := dwGetStr(joConfig,'table','');
            if (sText = '') or (Pos('#',sText)>0) then begin
                joConfig.new    := 0;
                joConfig.edit   := 0;
                joConfig.delete := 0;
                joConfig.import := 0;
            ...
            //生成一个主查询 (不读数据, 仅用于处理字段类型等)
            oFQMain                     := TFDStoredProc.Create(APanel);
            ...
            if sFields = '' then begin
                ...
                for iField := 0 to oFQMain.Fields.Count - 1 do begin
                    //取得
                    oFieldType  := oFQMain.Fields[iField].DataType;

                    joField := _json('{}');

                    //字段名
                    joField.name := oFQMain.Fields[iField].FieldName;
                    ...
                    end else if (oFieldType in csstInteger) or (oFieldType in csstFloat) then begin
                        //宽度
                        joField.width       := '120';
        ...
        if Result = 0 then begin
            try
                ...
                for iField := 0 to joConfig.fields._Count - 1 do begin
                    ...
                    //设置datatype
                    joField.datatype := oFQMain.Fields[iFieldId].DataType;
                end;
            except
                //异常时调试使用
                Result  := -3;
            ...
            try
                ...
                for iField := 0 to joConfig.fields._Count - 1 do begin
                    ...
                    if joField.type = 'boolean' then begin              //--------------------
                        ...
                        joField.treelist    := csDBToTreeList(
                                oFQTemp,
                                joField.name,
                                joField.datafield,
                                joField.datafield,
                        ...
                        //创建树形字段的选项，参数分别为：查询控件、表名、数据存储字段、数据显示字段
                        joField.list    := csDBToTreeList(oFQTemp,joField.name,joField.datafield,joField.viewfield,
                                dwGetStr(joField,'listlink',' - '),    //连接字符串
                                dwGetInt(joField,'listcount',500));
                        joField.treelist    := csDBToTreeList(oFQTemp,joField.name,joField.datafield,joField.viewfield,
                                '',    //连接字符串,专门设置为空
                                dwGetInt(joField,'listcount',500));
                        ...
                        oFQTemp.Close;
                        if dwGetStr(joField,'listorder','')='' then begin
                            ////oFQTemp.SQL.Text := 'SELECT Distinct '+joField.datafield+' FROM '+joField.name;
                        end else begin
                            ////oFQTemp.SQL.Text := 'SELECT Distinct '+joField.datafield+' FROM '+joField.name+' ORDER By '+joField.listorder;
                        end;
                        oFQTemp.Open;
                        ...
                        oFQTemp.Close;
                        if dwGetStr(joField,'listorder','')='' then begin
                            ////oFQTemp.SQL.Text := 'SELECT Distinct '+joField.datafield+','+joField.viewfield+' FROM '+joField.name;
                        end else begin
                            ////oFQTemp.SQL.Text := 'SELECT Distinct '+joField.datafield+','+joField.viewfield+' FROM '+joField.name+' ORDER By '+joField.listorder;
                        end;
                        oFQTemp.Open;
                    ...
                    if (dwGetInt(joField,'dbfilter',0)=1)  then begin
                        ...
                        if TFieldType(joField.datatype) in [ftMemo] then begin
                            //不能对ftmemo取唯一值
                            joField.dbfilter := 0;
                        end else if  Pos('#',joConfig.table) > 0 then begin
                            //临时表取唯一值
                            oFDMem := TFDMemTable.Create(nil);
                            //
                            slFilter            := TStringList.Create;
                            slFilter.Sorted     := True;
                            slFilter.Duplicates := dupIgnore;

                            try
                                // 将存储过程结果复制到内存表中
                                oFDMem.Data     := oFQMain.Data;

                                // 使用Aggregates获取distinct值
                                oFDMem.Indexes.Clear;
                                with oFDMem.Indexes.Add do begin
                                    Name    := 'DistinctIndex';
                                    Fields  := String(joField.name);
                                    Active  := True;
                                end;
                                oFDMem.IndexName := 'DistinctIndex';

                                //
                                oFDMem.First;
                                while not oFDMem.Eof do begin
                                    slFilter.Add(Trim(oFDMem.FieldByName(String(joField.name)).AsString));
                                    oFDMem.Next;
                                end;

                                //
                                joField.filterlist  := _json('[]');
                                for iTemp :=0 to slFilter.Count - 1 do begin
                                    joField.filterlist.Add(slFilter[iTemp]);
                                end;
                            finally
                                slFilter.Free;
                                oFDMem.Free;
        ...
        if Result = 0 then begin
            try
                ...
                iCount  := 0;
                if joConfig.Exists('params') then begin
                    for iField := 0 to joConfig.params._Count-1 do begin
                        //取得当前字段JSON对象
                        joField := joConfig.params._(iField);
                        //如果字段加入分字段查询
                ...
                if iCount > 0 then begin
                    ...
                    if joConfig.Exists('params') then begin
                        for iField := 0 to joConfig.params._Count-1 do begin
                            ...
                            //如果字段加入分字段查询
                            if joField.query = 1 then begin
                                //参数 APanel:TPanel;AConfig:Variant;AField:Variant;AIndex:Integer;ASuffix:String;AFlowPanel:TFlowPanel
                                csCreateFieldQuery(APanel,joConfig,joField,1000+iField,IntToStr(iCount),oFQy);
                                //
                                Inc(iCount);
                            end;
                        end;
                    ...
                    for iField := 0 to joConfig.fields._Count-1 do begin
                        ...
                        if joField.query = 1 then begin
                            //
                            csCreateFieldQuery(APanel,joConfig,joField,iField,IntToStr(iCount),oFQy);
                            Inc(iCount);
                        end;
                    end;
                ...
                csSetQueryMode(APanel, dwGetInt(joConfig,'defaultquerymode',1),False);
        ...
        if Result = 0 then begin
            try
                ...
                with oSgD do begin
                    ...
                    ColCount        := dwIIFi(dwGetInt(joConfig,'select',0)=0,0,1)+csGetViewFieldCount(joConfig.fields); //列数
                    FixedCols       := 0;                       //固定列数
                    //
                    Font.Color      := $00969696;
                    //
        ...
        if Result = 0 then begin
            try
                ...
                if dwGetInt(joConfig,'defaultempty',0) <> 1 then begin
                    csUpdate(APanel,'',False);
                end else begin
                    csUpdate(APanel,'1=0',False);
                end;
            except
...
begin
    try
        ...
        //销毁临时表
        oFDQuery    := TFDStoredProc(APanel.FindComponent(sPrefix+'FQTemp'));
        if oFDQuery <> nil then begin
            FreeAndNil(oFDQuery);
        end;
