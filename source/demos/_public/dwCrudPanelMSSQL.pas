...
------------------------------------------------------------------------------------------------------------------------
### 2025-07-26
1. 从原dwCrudPanel中另存, 试图使用链集分页技术解决100万以上级别数据的分页问题
...
begin
    ...
    if APageNo = 0 then begin   //处理第一页
        //
        if ADatabase = 'mssql' then begin
            Result  := 'SELECT TOP '+IntToStr(APageSize)+' '+AFields+' FROM '+ATable;
        ...
        if iDiff>0 then begin
            ...
            //
            if ADatabase = 'mssql' then begin
                Result  := 'SELECT TOP '+IntToStr(APageSize*abs(iDiff))+' '+AFields+' FROM '+ATable;
...
function cpGetDataToGrid2(APanel:TPanel;AWhere,AOrder:String;APageNo:Integer):Integer;
var
...
begin
    ...
    //取得对应的参数设置
    sTable      := joConfig.table;       //表名
    sWhere      := AWhere;       //过滤条件
    sOrder      := AOrder;       //排序
    iPageNo     := APageNo;      //页码，从0始
    iPageSize   := joConfig.pagesize;    //每页行数
    sFields     := cpGetFields(joConfig.fields,False);  //得到字段字符串列表
...
(*
    ...
    oFDQuery.Close;
    oFDQuery.SQL.Text := 'SELECT Count(* ) AS AA FROM '+sTable+' '+sWhere;
    oFDQuery.Open;

    //记录总数
    iRecCount   := oFDQuery.Fields[0].AsInteger;
    ...
    //取当前页数据
    oFDQuery.Close;
    oFDQuery.SQL.Text := 'SELECT TOP '+IntToStr(iPageSize)+' '+ sFields+' FROM '+sTable+' '+sWhere+' '+sOrder;
    oFDQuery.FetchOptions.RecsSkip    := 0;
    oFDQuery.FetchOptions.RecsMax     := iPageSize;
    oFDQuery.Open;
    ...
    if AQuery.IsEmpty and (iPageNo > 0) then begin
        ...
        //重新取当前页数据
        oFDQuery.Close;
        oFDQuery.SQL.Text := 'SELECT '+ sFields+' FROM '+sTable+' '+sWhere+' '+sOrder;
        oFDQuery.FetchOptions.RecsSkip    := 0;
        oFDQuery.FetchOptions.RecsMax     := iPageSize;
        oFDQuery.Open;
    end;
    ...
    //如果超出最大页数,则为最大页数
    if iPageNo > Ceil(oTbP.Max /Math.Max(1,oTbP.PageSize)) then begin
        iPageNo := Ceil(oTbP.Max /Math.Max(1,oTbP.PageSize));
    end;
...
begin
    ...
    //取当前页数据
    AQuery.Close;
    AQuery.SQL.Text := 'SELECT TOP '+IntToStr(iPageSize)+' '+ sFields+' FROM '+sTable+' '+sWhere+' '+sOrder;
    AQuery.FetchOptions.RecsSkip    := 0;
...
begin
    try
        ...
        //取得主键
        sPrimaryKey := dwGetStr(joConfig,'primarykey','');
