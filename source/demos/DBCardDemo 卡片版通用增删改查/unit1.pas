...
begin
    //
    joConfig    := _json(Panel1.Hint);
    joConfig.table := '(SELECT tEmployeeID, SUM(tprice*tquantity) AS tTotalSales FROM ttSales'
            +' WHERE tSaleDate >= '''+FormatDateTime('YYYY-MM-DD',DateTimePicker1.Date)+''' AND tSaleDate < ''2025-09-01'''
            +' GROUP BY tEmployeeID) as A000';
