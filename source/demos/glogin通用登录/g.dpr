...
begin
    ...
    *)
    ...
    with Form1 do begin
        ...
        //加载图片
        I_Background.Align  := alClient;
        I_BackGround.Hint   := '{"src":"'+gjoConfig.background+'"}';

        //
        I_Logo.Hint     := '{"src":"'+gjoConfig.logo+'"}';

        //默认用户名
        if gjoConfig.Exists('defaultusername') then begin
            Et1.Text := gjoConfig.defaultusername;
        end;

        //默认密码
        if gjoConfig.Exists('defaultpassword') then begin
            E_Password.Text := gjoConfig.defaultpassword;
        end;


        //记住登录信息
        P_Remember.Visible  := gjoConfig.remember = 1;

        //验证码
        case gjoConfig.captcha of
            //不显示验证码
            0 : begin
                P_Captcha.Visible   := False;
            end;
            //显示验证码
            1 : begin
                P_Captcha.Visible   := True;
            end;
            //初次不显示验证码，输入错误后显示验证码
            2 : begin
                //取得错误次数
