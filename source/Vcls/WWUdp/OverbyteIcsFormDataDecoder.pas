...
Jan 22, 2024 V9.1  Moved TextToHtmlText and IcsHtmlValuesToUnicode to new unit
                     OverbyteIcsHtmlUtils to avoid circular references.
                   Reworked so content value or data is saved during Decode
                     as RawContentData except for file content, rather then
                     being read from the stream on demand.  If the encoding is
                     known it may be read as RawContentData and decoded rather
                     then using AsString which tries various decoding options.
                   Added property DecodeInfo that returns same output as Display
                      event debug but as a string.
                   Added property FormCodePage that may be set with the form
                     code page before decoding, used to correctly decode values.
                   Supported character set according to RFC7578 with an optional
                      _charset_ part typically specifying utf-8, sets FormCodePage.
                   If Content-Type specifies a charset or FormCodePage is set,
                     values are converted from that charset, otherwise % is checked
                     for percent encoding, or HTML encoding is assumed.
                   Added more PartData overrides that take a part number.
                   Display debug now includes textual data values.
                   Detect end of form if missing final -- missing after last
                     boundary, don't return illegal parts.

Note, this unit takes the Mime Boundary or Delimiter from the first line of the
MIME content, not the multipart/form-data; boundary=xx header, so ignores --
at the start of the boundary..

Beware FormDcoding may take a long time for content of a GB or more, which will
block the web server unless run in a thread, take a long time to copy file streams.

Note: character set sets in forms are poorly designed, Edge/Chromr and Firefox
send partially HTML encoded textual values
...
const
    FormDataDecoderVersion = 901;
    CopyRight : String      = ' FormDataDecoder (c) 2003-2023 F. Piette V9.1 ';
...
type
    ...
    {$ENDIF}
        ...
        FContentData          : RawByteString;    { V9.1 }
        FCodePage             : LongWord;         { V9.1 }
    public
    ...
    {$ENDIF}
        ...
        property RawContentType: RawByteString        read  FContentType
                                                      write FContentType;
        property RawContentData: RawByteString        read  FContentData
                                                      write FContentData;       { V9.1 }
    ...
    protected
        FData       : TStream;
        FIndex      : TList;
        FDelimiter  : AnsiString;            { aka MimeBoundary }
        FOnDisplay  : TDisplayEvent;
        FDataStream : TStream;
        FOwnData    : Boolean;
        FDataString : RawByteString;
        FDecodeInfo : String;                   { V9.1 }
        FFormCodePage : LongWord;               { V9.1 }
    ...
    {$IFDEF COMPILER12_UP}
        ...
        function    PartData(const PartNumber   : Integer) : RawByteString; overload;     { V9.1 }
        ...
        function    PartData(const PartNumber   : Integer;
                             ASrcCodePage       : LongWord = CP_ACP;
                             const ADetectUtf8  : Boolean = TRUE;
                             ADetectHtml        : Boolean = FALSE) : UnicodeString; overload;    { V9.1 }
    {$ELSE}
        function    PartData(const PartName : AnsiString): AnsiString; overload;
        function    PartData(const PartNumber : Integer): AnsiString; overload;               { V9.1 }
    {$ENDIF}
    ...
    published
        property FormCodePage  : LongWord  read  FFormCodePage write FFormCodePage; { V9.1 }
        property OnDisplay : TDisplayEvent read  FOnDisplay write FOnDisplay;
        property DecodeInfo : String read FDecodeInfo;                   { V9.1 }
    end;

    function DocumentToContentType(const FileName : String) : String;        { V8.49 }

(* V9.1 moved to CharsetUtils to avoid circular references
...
begin
    FDecodeInfo := FDecodeInfo + Msg + IcsCRLF;         { V9.1 }
...
begin
    ...
    while TRUE do begin
        // Allocate a new blank item and add it to the list
//        Display('Start of Header, StreamPos=' + IntToStr(FData.Position) + ' of ' + IntToStr(FData.Size));
        ...
        while TRUE do begin
            ...
            if Ch <> FDelimiter[I] then begin
                ...
                I := 1;
                Continue;    // not found complete delimiter
            end;
 ...
 //       Display('End of data: PosHeader=' + IntToStr(Item.PosHeader) + ', PosData=' + IntToStr(Item.PosData) +
//                                               ', StreamPos=' + IntToStr(FData.Position) + ' of ' + IntToStr(FData.Size));

     { V9.1 sanity check for empty item, remove it from list }
        if (Item.DataLength < 0) or (Item.RawContentName = '') then begin
            FIndex.Delete(FIndex.Count - 1);
            break;
        end;

     { V9.1 keep raw value, unless filename }
        if (Item.ContentFileName <> '') or (Item.DataLength <= 0) then
            Item.FContentData := ''
        else begin
            SetLength(Item.FContentData, Item.DataLength);
            OldPos := FData.Position;
            FData.Position := Item.PosData;
            FData.Read(Pointer(Item.FContentData)^, Item.DataLength);
            FData.Position := OldPos;
        end;

    //    if Assigned(FOnDisplay) then begin    { V9.1 build local diag string as well as event  }
        Display('Part ' + IntToStr(FIndex.Count));
        Display('  Name: ' + Item.ContentName);
        if Item.ContentFileName <> '' then
            Display('  FileName: ' + Item.ContentFileName);
        Display('  DataLength: ' + IntToStr(Item.DataLength));
        if Item.ContentType <> '' then
            Display('  ContentType: ' + Item.ContentType);  { may include charset= }
        if Item.FContentData <> '' then
            Display('  Value: ' + Item.AsString);       { V9.1 show value }

     { V9.1 keep form character set part, per RFC7578, probably utf-8 }
        if Item.ContentName = '_charset_' then begin
            if MimeCharsetToCodePage(String(Item.FContentData), ACodePage) then
                FFormCodePage := ACodePage; // replace default
        end;

     { V9.1 contemt=type may also supply character set }
        if Item.ContentType <> '' then begin
            if MimeCharsetToCodePage(String(Item.FContentType), ACodePage) then
                Item.FCodePage := ACodePage;
        end;

     { boundary must be followed by CRLF if more parts, '--' used for end of form }
...
begin
    ...
    if not Assigned(Item) then
        ...
        Result := Item.AsRawByteString;   { V9.1 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TFormDataAnalyser.PartData(const PartNumber : Integer) : RawByteString;   { V9.1 }
var
    Item : TFormDataItem;
begin
...
begin
    ...
    if not Assigned(Item) then
        ...
        Result := Item.AsString(ASrcCodePage, ADetectUtf8, ADetectHtml);  { V9.1 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TFormDataAnalyser.PartData(const PartNumber : Integer) : AnsiString;   { V9.1 }
var
    Item : TFormDataItem;
begin
    Item := Part(PartNumber);
...
begin
    Clear;
    FOwnData    := TRUE;
    FDataStream := TIcsBufferedStream.Create(FileName, fmOpenRead or fmShareDenyWrite, 4096);
    InternalDecodeStream(FDataStream);
end;
...
begin
    ...
    Analyser.FData.Position := PosData;      { V9.1 }
...
begin
    Result := FContentData;   { V9.1 }
//    SetLength(Result, DataLength);
//    Analyser.FData.Position := PosData;
//    Analyser.FData.Read(Pointer(Result)^, DataLength);
end;
...
//    if True then
    Result := '';
    if FContentData = '' then
        Exit;
    DetectHtml := ADetectHtml;
    if (Pos('&#', String(FContentData)) > 0) then // HTML endoded characters
       DetectHtml := True;
 { content-type included the charset }
    if FCodePage <> 0 then
        Result := AnsiToUnicode(FContentData, FCodePage)
 { we had a _charset_ part or have been told the charset }
    else if Analyser.FFormCodePage <> 0 then
        Result := AnsiToUnicode(FContentData, Analyser.FFormCodePage)
    else if (NOT DetectHtml) and (Pos('%', String(FContentData)) > 0) then // percent endoded characters
        Result := AnsiToUnicode(IcsPercentDecode(FContentData))
    else
        Result := IcsHtmlValuesToUnicode(FContentData, ASrcCodePage, ADetectUtf8, DetectHtml);   { V9.1 }
end;
{$ELSE}
