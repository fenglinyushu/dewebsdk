...
- add an inizialization section where you call THttpContCodHandler.RegisterContentCoding
...
Oct 20, 2022 V8.70 no changes here, but OverbyteIcsHttpCCodZLib rewritten.
Dec 16, 2022 V8.71 Added support to enumerate supported codings in THttpContCodHandler,
...
type
    ...
    public
        procedure WriteBuffer(Buffer: Pointer; Count: Integer); override;   { V8.71 JK }
    end;
    ...
    public
        ...
        function Count: Longint;                                      { V8.71 JK }
        function GetItem( Index : Longint ): THttpContCodItem;        { V8.71 JK }
...
begin
    if Pos(BASE_CLASS_NAME, ClassName) = 1 then
        ...
        raise EHttpContentCodingException.CreateFmt(ERR_GETCODING_OVERRIDE, [ClassName]);
end;
...
begin
     // overridden with real code to uncompress stream
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpCCodIdentity.WriteBuffer(Buffer: Pointer; Count: Integer);   { V8.71 JK }
begin
  OutputWriteBuffer( Buffer, Count );
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure THttpCCodStar.WriteBuffer(Buffer: Pointer; Count: Integer);     { V8.71 JK }
begin
  OutputWriteBuffer( Buffer, Count );
end;
            ...
            if ContItem.Enabled then begin
                if AddQuality then begin
                    ...
                    if FHeaderText = '' then
                        ...
                        FHeaderText := Format('%s, %s;q=%s', [FHeaderText, ContItem.Coding, QualStr]);
                end
                else if ContItem.Quality > 0 then begin
                    if FHeaderText = '' then
                        FHeaderText := ContItem.Coding
                    else
                        FHeaderText := Format('%s, %s', [FHeaderText, ContItem.Coding]);
                end;
            end;
        end;
        FRecalcHeader := FALSE;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function THttpContCodHandler.Count: Longint;                                  { V8.71 JK }
begin
  Result := FCodingParams.Count;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function THttpContCodHandler.GetItem( Index : Longint ): THttpContCodItem;     { V8.71 JK }
begin
  Result := nil;
  if Index < 0 then Exit;
  if Index >= FCodingParams.Count then Exit;
  Result := FCodingParams.Items[Index];
end;
