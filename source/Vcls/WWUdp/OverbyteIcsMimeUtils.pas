...
Dec 16, 2022 V8.71 EncodeQuotedPrintable has new Specials TSysCharSet parameter to
                     allow special encoding of selected characters.
                   Added TTransparentStream and InitStreamEncBase64 for use adding
                     MIME stream attachments (instead of files), used by THtmlSmtpCli
...
type
  TTransparentStream = class(TStream)           { V8.71 JK }
  protected
  ...
  public
    ...
    constructor Create( AOriginalStream : TStream );
...
{ V8.71 JK added "Specials : TSysCharSet" to parameters due possibility encoding selected chars (delimiters) }
function  EncodeQuotedPrintable(const S: RawByteString; Specials: TSysCharSet = []): String; overload;
{$IFDEF COMPILER12_UP}
function  EncodeQuotedPrintable(const S: UnicodeString; ACodePage: LongWord; Specials: TSysCharSet = []): UnicodeString;  overload;
function  EncodeQuotedPrintable(const S: UnicodeString; Specials: TSysCharSet = []): UnicodeString; overload;
{$ENDIF}
...
{ V8.71 JK see description in class TTransparentStream }
...
{V8.71 JK TTransparentStream  }
constructor TTransparentStream.Create( AOriginalStream : TStream );
begin
    inherited Create;
    FStream := AOriginalStream;
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TTransparentStream.SetSize(NewSize: Longint);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TTransparentStream.SetSize(const NewSize: Int64);
begin
  FStream.Size := NewSize;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TTransparentStream.Read(var Buffer; Count: Longint): Longint;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TTransparentStream.Write(const Buffer; Count: Longint): Longint;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TTransparentStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TTransparentStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
  Result := FStream.Seek( Offset, Origin );
end;
...
{ V8.71 JK added "Specials : TSysCharSet" to parameters due possibility encoding selected chars (delimiters) }
function EncodeQuotedPrintable(const S: RawByteString; Specials: TSysCharSet = []): String;
var
    I, J : Integer;
begin
    Result := '';
    ...
    while I <= Length(S) do begin
        ...
        while (I <= Length(S)) and
              {$IFDEF UNICODE}
              (not CharInSet(S[I], Specials)) and
              {$ELSE}
              (not (S[I] in Specials)) and
              {$ENDIF}
              (S[I] <> '=') and
              (S[I] >= ' ') and
              (Ord(S[I]) <= 126) do
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF COMPILER12_UP}
function EncodeQuotedPrintable(const S: UnicodeString; ACodePage: LongWord; Specials: TSysCharSet = []): UnicodeString;
begin
    Result := EncodeQuotedPrintable(UnicodeToAnsi(S, ACodePage), Specials);
end;


{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function EncodeQuotedPrintable(const S: UnicodeString; Specials: TSysCharSet = []): UnicodeString;
begin
    Result := EncodeQuotedPrintable(S, CP_ACP, Specials);
end;
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function InitStreamEncBase64(AStream: TStream): TStream;      { V8.71 JK }
begin
...
{ V8.71 JK Stream class may have other parent class than TIcsBufferedFileStream }
    if Stream is TIcsBufferedFileStream then begin
    ...
    else  { if Stream is not TIcsBufferedFileStream... }
        More := Stream.Position < Stream.Size;
{$ELSE}
