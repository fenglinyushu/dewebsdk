...
Legal issues: Copyright (C) 2024 by François PIETTE and Author
...
Jan 20, 2024 V9.1  Now build on Android.
Sep 16, 2024 V9.3  Added OverbyteIcsTypes for consolidated types and constants.
                   Added OverbyteIcsUtils for IP functions
                   Added NativeInt version of TInterfaceList.Items[] due to
                     a Win64 TList change in Delphi 12.
Oct 03, 2024 V9.4  IPv4 address now Longword not Integer.
...
{$IFDEF TListNatInt}
        function  GetItem(Index: NativeInt): PIcsInterfaceInfo;             { V9.3 Delphi 12.2 changed type }
        procedure SetItem(Index: NativeInt; IInfo: PIcsInterfaceInfo);
{$ELSE}
...
{$IFDEF TListNatInt}
        property  Items[Index: NativeInt]: PIcsInterfaceInfo read GetItem write SetItem; default; { V9.3 Delphi 12.2 changed type }
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF TListNatInt}
function TInterfaceList.GetItem(Index: NativeInt): PIcsInterfaceInfo;             { V9.3 Delphi 12.2 changed type }
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF TListNatInt}
procedure TInterfaceList.SetItem(Index: NativeInt; IInfo: PIcsInterfaceInfo);    { V9.3 Delphi 12.2 changed type }
begin
...
begin
    if getifaddrs(ifap) <> 0 then
        raise ESocketException.Create('IcsGetInterfaceList getifaddrs(), Error: ' + SysErrorMessage(GetLastError));
    try
      Next := ifap;
      while Next <> nil do begin
        if (Next.ifa_flags and IFF_UP = IFF_UP) (* and
           ...
           ((Next.ifa_addr.sa_family = AF_INET) or            { V9.1 !!! sa_famly missing }
              ((NOT SkipIPv6) and (Next.ifa_addr.sa_family = AF_INET6))) *) then   { V8.64 }
        begin
            New(PInfo);
            FillChar(PInfo^, SizeOf(TIcsInterfaceInfo), 0);
            PInfo^.iiFlags := Next.ifa_flags;
         (*   if Next.ifa_addr.sa_family = AF_INET then begin         { V9.1 !!! sa_famly missing }
       ...
       IcsGetInterfaceList(iList, True);  { V8.64 don't need IPv6 addresses }
        ...
        begin
            ...
            if iInfo.iiAddress.Address.sa_family <> AF_INET then
                Continue;
            IFS_addr := iInfo.iiAddress.addressIn.sin_addr.S_addr and iInfo.iiNetMask.addressIn.sin_addr.S_addr;
