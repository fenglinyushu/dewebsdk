...
Jan 05, 2023 - V8.71 - Several improvements by Vitaly Volkov:
                       The creation and deletion code is framed in try-finally.
                       Fixed memory leaks.
                       Added Sec-WebSocket-Protocol response.
                       TWebSocketMessage event returns OpCode as TextFrame or BinaryFrame.
                       sendString method now has OpCode argument defaulting to
                         TextFrame but can be changed to BinaryFrame.
...
that supports SSL and is integrated into the OverbyteIcsSslMultiWebServ sample. There is
also a new WebSocket client component unit OverbyteIcsWebSocketCli that supports SSL and
is integrated into the OverbyteIcsHttpRest sample.
...
type
  ...
  IWebSocketMessage = class
     ...
     * @param AnsiString data
     ...
     procedure setData(data: AnsiString; OpCode: TWebSocketOpcode = TextFrame); virtual; abstract;    { V8.71 added OpCode }
    {
    ...
    function getType: TWebSocketOpcode; virtual; abstract;           { V8.71 }
    {
  ...
  IWebSocketConnection = class
    ...
    procedure sendString(msg: AnsiString; OpCode: TWebSocketOpcode = TextFrame); virtual; abstract;  { V8.71 added OpCode }
  ...
  TWebSocketMessage = procedure(Sender: TObject; Msg: String; OpCode: TWebSocketOpcode) of object;   { V8.71 added OpCode }
  ...
  TWebSocketMessageHybie = class (IWebSocketMessage)
    public
      constructor Create(data: AnsiString; OpCode: TWebSocketOpcode = TextFrame);    { V8.71 added OpCode }
      destructor Destroy; override;
      procedure createFrames(OpCode: TWebSocketOpcode = TextFrame);    { V8.71 added OpCode }
      {
      ...
      procedure setData(data: AnsiString; OpCode: TWebSocketOpcode = TextFrame); override;    { V8.71 added OpCode }
      function getData: AnsiString; override;
      function getType: TWebSocketOpcode; override;           { V8.71 }
  ...
  TWebSocketConnection = class (IWebSocketConnection)
    public
      ...
      procedure sendString(msg: AnsiString; OpCode: TWebSocketOpcode = TextFrame); override;  { V8.71 added OpCode }
...
constructor TWebSocketMessageHybie.Create(data: AnsiString; OpCode: TWebSocketOpcode = TextFrame);    { V8.71 added OpCode }
begin
...
procedure TWebSocketMessageHybie.createFrames(OpCode: TWebSocketOpcode = TextFrame);    { V8.71 added OpCode }
var
  t: TWebSocketFrame;
begin
  t := TWebSocketFrameHybie.Create(OpCode, _data);    { V8.71 was TextFrame }
  _frames.Add(t);
end;
...
procedure TWebSocketMessageHybie.setData(data: AnsiString; OpCode: TWebSocketOpcode = TextFrame);    { V8.71 added OpCode }
begin
  _data := data;
  createFrames(OpCode);
end;
...
function TWebSocketMessageHybie.getType: TWebSocketOpcode;            { V8.71 }
begin
  Result := _frames.Frame[0].getType;
end;
...
constructor TWebSocketMessageHixie.Create(data: AnsiString; OpCode: TWebSocketOpcode = TextFrame);    { V8.71 added OpCode }
begin
  if data <> '' then
    setData(data, OpCode);
end;
...
procedure TWebSocketMessageHixie.setData(data: AnsiString; OpCode: TWebSocketOpcode = TextFrame);  { V8.71 added OpCode }
var
...
function TWebSocketMessageHixie.getType: TWebSocketOpcode;            { V8.71 }
begin
  Result := _frame.getType;
end;
...
begin
  if assigned(_con) and assigned(_socket.OnWebSocketMessage) then
    _socket.OnWebSocketMessage(self, String(m.getData), m.getType);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // V8.71 Volkov add try-finally-free
      SL := TWebSocketFunctions.cookie_parse(AnsiString(_headers.ValueFromIndex[i]));
      try
        _cookies.AddStrings(SL);
      finally
        SL.Free;
      end;
    end;
...
procedure TWebSocketConnection.sendString(msg: AnsiString; OpCode: TWebSocketOpcode = TextFrame);   { V8.71 added OpCode }
begin
end;
...
begin
  ...
  // V8.71 Volkov 19/09/2022
  if _headers.Values['Sec-WebSocket-Protocol'] <> '' then
    response := response + 'Sec-WebSocket-Protocol: ' + AnsiString(_headers.Values['Sec-WebSocket-Protocol']) + #13#10;
...
procedure TWebSocketConnectionHybie.sendString(msg: AnsiString; OpCode: TWebSocketOpcode = TextFrame);   { V8.71 added OpCode }
var
  m: TWebSocketMessageHybie;
begin
  m := TWebSocketMessageHybie.Create(msg, OpCode);
...
procedure TWebSocketConnectionHixie.sendString(msg: AnsiString; OpCode: TWebSocketOpcode = TextFrame);   { V8.71 added OpCode }
var
  m: IWebSocketMessage;
begin
  m := TWebSocketMessageHixie.Create(msg, OpCode);
