...
uses
    //第三方单元
    SynCommons,     //JSON解析单元
    SynEditHighlighter,SynEditCodeFolding, SynHighlighterJSON, SynEdit, //语法高亮单元
...
begin
    try
        joData      := _json(Trim(SynEdit.Text));
        //
        //此处需要格式化
        SynEdit.Lines.Text := JSONReformat(joData);
    except
    end;
end;
...
begin
    ...
    try
        with MainForm do begin
            ...
            joData      := _json(sText);
            if joData <> unassigned then begin
                SynEdit.Text    := JSONReformat(joData);
            end else begin
                SynEdit.Text    := sText;
            end;
