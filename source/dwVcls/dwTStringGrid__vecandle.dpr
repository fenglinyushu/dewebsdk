    ...
    with TStringGrid(ACtrl) do begin
        ...
        with TStringGrid(ACtrl) do begin
            ...
            sCode   := dwFullName(Actrl)+'__dat : {'#13
                    +'    columns: ['''+Cells[0,0]+''', '''+Cells[1,0]+''', '''+Cells[2,0]+''', '''+Cells[3,0]+''', '''+Cells[4,0]+''', '''+Cells[5,0]+'''],'#13
                    +'    rows: [';

            for iRow := 1 to RowCount-1 do begin
                 //sCode     := sCode + Format(#13'        {'''+Cells[0,0]+''':''%s'','''+Cells[1,0]+''':%s, '''+Cells[2,0]+''':%s, '''+Cells[3,0]+''':%s, '''+Cells[4,0]+''':%s, '''+Cells[5,0]+''':%s},',
                 //       [Cells[0,iRow],Cells[1,iRow],Cells[2,iRow],Cells[3,iRow],Cells[4,iRow],Cells[5,iRow]]);

                 //以下是简化格式
                 sCode     := sCode + Format(#13'        [''%s'',%s,%s,%s,%s,%s],',
                        [Cells[0,iRow],Cells[1,iRow],Cells[2,iRow],Cells[3,iRow],Cells[4,iRow],Cells[5,iRow]]);
            end;
