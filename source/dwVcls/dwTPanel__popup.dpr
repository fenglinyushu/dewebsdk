...
begin
    with TPanel(ACtrl) do begin
        ...
        if joData.e = 'onclick' then begin
            OnClick(TPanel(ACtrl));
        end else if joData.e = 'onenter' then begin
            OnEnter(TPanel(ACtrl));
        end else if joData.e = 'onexit' then begin
            OnExit(TPanel(ACtrl));
        end else if joData.e = 'onbkclick' then begin
            TPanel(ACtrl).Visible   := False;
...
begin
    ...
    with TPanel(ACtrl) do begin
        sCode   := sFull + '__bkclick(e)'+
                '{' +
                    sFull+'__vis = false;' +
                    'this.dwevent(null,"'+sName+'","",''onbkclick'','''+IntToStr(TForm(Owner).Handle)+''');'
                +'},';
        joRes.Add(sCode);
    end;
